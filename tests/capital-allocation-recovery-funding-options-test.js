...
const source=fs.readFileSync(path.join(__dirname,'../js/strategy-balance.js'),'utf8'),fundingLoader=source.indexOf("capital-allocation-recovery-funding.js','8D-13"),optionsLoader=source.indexOf("capital-allocation-recovery-funding-options.js','8D-15");assert.ok(fundingLoader>=0&&optionsLoader>fundingLoader,'funding options must load after recovery funding');assert.equal((source.match(/capital-allocation-recovery-funding-options\.js/g)||[]).length,1);
