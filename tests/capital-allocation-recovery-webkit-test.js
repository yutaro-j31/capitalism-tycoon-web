...
function installCapitalAllocationModules(load) {
  for (const file of [
    ...
    'capital-allocation-recovery-funding-options.js',
...
function resilientPublicSave() {
  ...
  state.companyCash = 2_000_000_000;
  state.companyDebt = 100_000_000;
  state.companyCredit = 100;
  state.companyReputation = 80;
  state.investorConfidence = 80;
  state.sharesOut = 1_000_000;
  state.founderShares = 600_000;
  state.stockPrice = 1_000;
  state.ticker = 'CPTY';
  ...
  property.value = 1_000_000_000;
  property.price = 1_000_000_000;
  property.purchasePrice = 600_000_000;
  property.bookValue = 600_000_000;
  ...
  assert.deepEqual(finance.validate(game.g).errors, []);
...
async function assertMobileCardLayout(page, selector) {
  ...
  assert.ok(layout.controls.length >= 4, 'recovery card must expose all option controls');
