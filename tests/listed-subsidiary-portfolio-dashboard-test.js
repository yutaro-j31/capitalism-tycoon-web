...
assert.equal(engine.setListedSubsidiaryPortfolioMandate('a','grow'),true);assert.equal(state.subsidiaries[0].portfolioMandateID,'grow');assert.deepEqual(engine.transactionCalls,[{type:'change',meta:{source:'listedSubsidiaryPortfolioMandate'}}]);assert.deepEqual({companyCash:state.companyCash,personalCash:state.personalCash,bookValue:state.subsidiaries[0].carryingBookValue,cumulativeProfit:state.accounting.cumulativeProfit},financialBefore);assert.equal(state.saveVersion,9);
