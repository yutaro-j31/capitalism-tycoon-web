...
const recoveryActionLoader = "loadPhaseScript('./js/capital-allocation-recovery-action.js','8D-9'";
...
assert.ok(strategySource.includes(resilienceLoader), 'the Phase 8D-5 resilience memo must be dynamically loaded in production');
assert.equal((strategySource.match(/capital-allocation-resilience-memo\.js/g) || []).length, 1, 'the Phase 8D-5 resilience memo must be wired exactly once');
assert.ok(strategySource.includes(recoveryActionLoader), 'the Phase 8D-9 recovery action must be dynamically loaded in production');
assert.equal((strategySource.match(/capital-allocation-recovery-action\.js/g) || []).length, 1, 'the Phase 8D-9 recovery action must be wired exactly once');
...
assert.ok(strategySource.indexOf(resilienceLoader) < strategySource.indexOf(recoveryActionLoader), 'the recovery action must load after the resilience recovery plan');
