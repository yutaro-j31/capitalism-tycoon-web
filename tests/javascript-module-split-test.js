...
assert(expected.at(-16) === './js/player-crisis-ui.js', 'player-crisis-ui.js must precede the engine bridge');
assert(expected.at(-15) === './js/player-engine-bridge.js', 'engine bridge must capture the app load');
assert(expected.at(-14) === './js/strategy-balance.js', 'strategy balance must precede progression balance');
assert(expected.at(-13) === './js/progression-balance.js', 'progression balance must precede app startup');
assert(expected.at(-12) === './js/app.js', 'app.js must compose the engine before post-app balance modules');
assert(expected.at(-11) === './js/difficulty-scenario-balance.js', 'difficulty scenario balance must precede crisis lifecycle');
assert(expected.at(-10) === './js/player-crisis.js', 'player-crisis.js must precede crisis actions');
assert(expected.at(-9) === './js/player-crisis-actions.js', 'player-crisis-actions.js must precede restructuring');
assert(expected.at(-8) === './js/player-crisis-restructuring.js', 'restructuring must precede creditor negotiation');
assert(expected.at(-7) === './js/player-crisis-creditor.js', 'creditor negotiation must precede weekly debt service');
assert(expected.at(-6) === './js/player-debt-service.js', 'weekly debt service must precede turnaround planning');
assert(expected.at(-5) === './js/player-turnaround-plan.js', 'turnaround planning must precede crisis UI extensions');
assert(expected.at(-4) === './js/player-crisis-creditor-ui.js', 'creditor UI must precede turnaround plan UI');
assert(expected.at(-3) === './js/player-turnaround-plan-ui.js', 'turnaround plan UI must precede weekly reporting');
assert(expected.at(-2) === './js/player-turnaround-plan-report.js', 'turnaround weekly report must precede release diagnostics');
assert(expected.at(-1) === './js/release-diagnostics-ui.js', 'release diagnostics must be the final external UI module');
