...
a.week=106;const secondary=engine.listedSubsidiaryFollowOnOfferingPreview('listed-1','secondary');assert.equal(secondary.secondaryProceeds,42009600);assert.equal(secondary.bookSold,24015000);assert.equal(secondary.gain,17994600);assert.equal(engine.executeListedSubsidiaryFollowOnOffering('listed-1','secondary'),true);
sub=a.subsidiaries[0];assert.equal(a.companyCash,92009600);assert.equal(sub.carryingBookValue,155985000);assert.equal(sub.ownership,0.649543);assert.equal(a.personalCash,beforePersonal);assert.equal(a.saveVersion,9);
