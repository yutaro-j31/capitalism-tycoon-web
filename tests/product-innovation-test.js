...
assert.equal(innovation.HISTORY_LIMIT, 80);
assert.equal(innovation.PROJECT_LIMIT, 120);
...
const roadmapTransaction = e.g.financeLedger.transactions.find(row => row.operationID === project.projectID);
assert.ok(roadmapTransaction, 'roadmap investment must reach the finance ledger');
assert.equal(roadmapTransaction.sourceType, 'productInnovationRoadmap');
...
const completedQuality = product.quality;
e.updateProductInnovationWeekly();
assert.equal(product.quality, completedQuality, 'completed effect must not be applied twice');
...
assert.ok(e.g.financeLedger.transactions.some(row => row.operationID === 'productPatent-innovation-patent-1'));

const html = innovation.renderSection(e);
assert.ok(html.includes('プロダクト・イノベーション'));
assert.ok(html.includes('成長SaaS'));
assert.ok(html.includes('Phase 8B-1'));
assert.ok(html.includes('data-product-innovation-action="start"'));

// Selling the product must terminate a live roadmap without refunding the investment.
e.g.productVentures.push({
  id: 'innovation-product-2', blueprintID: 'app', name: '売却対象SaaS', category: 'SaaS', status: 'released',
  progress: 100, quality: 40, brand: 25, users: 5000, paidUsers: 250, price: 2500,
  serverCost: 18000, serverCapacity: 20000, market: 1_000_000, risk: 0.1,
  valuation: 20_000_000, revenue: 1_000_000, cost: 500_000, profit: 500_000
});
e.ensureProductInnovationDefaults();
const secondProject = e.startProductInnovationRoadmap('innovation-product-2', 'quality_refresh');
assert.equal(secondProject.status, 'active');
e.sellProduct('innovation-product-2');
assert.equal(secondProject.status, 'cancelled');
assert.ok(e.g.productInnovationHistory.some(row => row.projectID === secondProject.projectID && row.type === 'roadmapCancelled'));
...
assert.deepEqual(findStateIssues(JSON.parse(JSON.stringify(e.g))), []);
