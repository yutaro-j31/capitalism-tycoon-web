...
assert.ok(html.includes('Phase 8A-3'));
...
const lifecycleCash = e.g.companyCash;
e.g.week = 30;
e.updateProductLifecycleWeekly();
assert.ok(e.g.companyCash < lifecycleCash, 'weekly maintenance should consume cash');
const standardDebt = product.technicalDebt;
assert.equal(e.setProductMaintenancePolicy(product.id, 'lean'), true);
e.g.week = 31;e.updateProductLifecycleWeekly();
assert.ok(product.technicalDebt > standardDebt, 'lean maintenance should grow technical debt');
...
console.log('product innovation and lifecycle checks passed');