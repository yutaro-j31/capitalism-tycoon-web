...
function installModules(load) {
  for (const file of [
    ...
    if (!load.modules[key]) {
      vm.runInContext(fs.readFileSync(path.join(ROOT, 'js', file), 'utf8'), load.ctx, { filename:file });
    }
  }
  return load;
}
...
function outcomeFixture() {
  ...
  const { engine, finance, capitalAllocationRecoveryFundingReadiness, capitalAllocationRecoveryFundingReconciliation } = load.modules;
  ...
  finance.event(state, 'revenue', 100_000_000, {
    ...
    sourceID:'baseline',
    idempotencyKey:'outcome-webkit-revenue',
    description:'Outcome WebKit fixture revenue'
  });
  state.finance.loans = [{
    id:'fixture-loan', loanID:'fixture-loan', status:'active',
  ...
  assert.equal(engine.SAVE_VERSION, SAVE_VERSION);
  assert.equal(game.g.saveVersion, SAVE_VERSION);

  const seedPlan = capitalAllocationRecoveryFundingReadiness.snapshot(game, TARGET_SCORE, 'recommended');
  game.g.finance.capitalAllocationPolicy.id = seedPlan.targetPolicy;
  game.g.finance.capitalAllocationPolicy.lastChangedWeek = -999;
  const candidates = capitalAllocationRecoveryFundingReconciliation.candidates(game, TARGET_SCORE);
  const preferred = candidates.find(row => row.ready && row.steps.some(step => ['securities', 'properties', 'borrowing'].includes(step.kind)));
  assert.ok(preferred, 'fixture must expose a ready target-50 plan with a funding transaction');
  assert.deepEqual(Array.from(finance.validate(game.g).errors || []), []);
...
async function main() {
  ...
  try {
    ...
    }, { key:SAVE_KEY, value:fixture.save });
    ...
    const pinned = await page.evaluate(({ targetScore, optionId }) => {
      ...
      return value ? {
        ...
        stepKinds:Array.from(value.steps || [], step => step.kind)
      } : null;
    }, { targetScore:fixture.targetScore, optionId:fixture.optionId });
    assert.ok(pinned, 'target-50 recovery plan must pin in the production browser runtime');
    ...
    fs.writeFileSync(RESULT_PATH, `${JSON.stringify({
      ...
      pinnedOption:fixture.optionId,
      executedStepKinds:fixture.expectedStepKinds,
      outcome:execution.report,
      saveKey:SAVE_KEY,
      saveVersion:SAVE_VERSION
    }, null, 2)}\n`);
    console.log('capital allocation recovery outcome iPhone WebKit workflow passed');
    await context.close();
  } catch (error) {
    if (page) {
      try { await page.screenshot({ path:path.join(OUT, 'capital-allocation-recovery-outcome-failure.png') }); } catch (_) {}
    }
    fs.writeFileSync(RESULT_PATH, `${JSON.stringify({
