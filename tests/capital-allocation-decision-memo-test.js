...
assert.equal(mod.classify({currentPolicy:'balanced',recommendedPolicy:'balanced',direct:true,executableAmount:5_000_000,actionImprovement:3,actionCanExecute:false,fundingGap:1_000_000}).id,'secureFunding');
...
const game=publicGame(modules);assert.deepEqual(modules.finance.validate(game.g).errors,[],'memo fixture must satisfy accounting invariants');game.g.finance.dirtyWeeks=[27];
const before=JSON.stringify(game.g),memoA=game.capitalAllocationDecisionMemo(),memoB=game.capitalAllocationDecisionMemo();
assert.deepEqual(memoA,memoB,'board memo must be deterministic');assert.equal(JSON.stringify(game.g),before,'board memo must not mutate save data, finance cache, dirty weeks, or accounting state');
...
assert.equal(memoA.threshold.executableAmount,memoA.threshold.scoreImpact.amount,'memo must use the threshold repayment preview');assert.deepEqual(memoA.impact,memoA.threshold.scoreImpact,'memo impact must be the threshold action impact');
...
assert.match(html,/data-capital-allocation-decision-memo/);assert.match(html,/取締役会・資本配分メモ/);assert.match(html,/推奨決定/);assert.match(html,/判断理由/);assert.match(html,/方針変更/);assert.match(html,/閾値返済/);if(!memoA.recovery.direct)assert.match(html,/対象外/);
