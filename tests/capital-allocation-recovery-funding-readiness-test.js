...
assert.deepEqual(snapA,snapB,'readiness snapshot must be deterministic');assert.equal(JSON.stringify(game.g),before,'readiness snapshot must remain read-only');assert.equal(snapA.targetScore,70);assert.equal(snapA.optionId,'recommended');assert.ok(snapA.fingerprint);assert.equal(snapA.baselineCash,game.g.companyCash);assert.equal(snapA.baselineDebt,game.g.companyDebt);assert.equal(snapA.baselineTransactionCount,game.g.finance.transactions.length);assert.equal(snapA.baselineLastTransactionId,game.g.finance.transactions.at(-1).transactionID);assert.ok(['ready','waitPolicy','blocked'].includes(snapA.status));assert.equal(snapA.ready,snapA.targetReached&&snapA.blockers.length===0);assert.ok(snapA.steps.length>=1);assert.deepEqual(snapA.steps.map(row=>row.order),snapA.steps.map((_,index)=>index+1));
...
const unchanged=game.validateCapitalAllocationRecoveryFundingSnapshot(snapA);assert.equal(unchanged.valid,true);assert.equal(unchanged.stale,false);assert.deepEqual(unchanged.reasons,[]);assert.equal(unchanged.currentFingerprint,snapA.fingerprint);assert.equal(JSON.stringify(game.g),before,'snapshot validation must remain read-only');
...
const financeGame=publicGame(modules),financeSnapshot=financeGame.capitalAllocationRecoveryFundingReadiness(70,'recommended');modules.finance.event(financeGame.g,'otherOperating',1,{cashEffect:0,profitEffect:0,sourceType:'readinessMarkerTest',sourceID:'marker',description:'marker'});const financeValidation=financeGame.validateCapitalAllocationRecoveryFundingSnapshot(financeSnapshot);assert.equal(financeValidation.stale,true);assert.ok(financeValidation.reasons.some(row=>row.id==='financeHistoryChanged'));
...
const html=mod.render(game);assert.equal(JSON.stringify(game.g),before,'readiness render must remain read-only');assert.match(html,/data-capital-allocation-recovery-funding-readiness/);assert.match(html,/回復資金・実行前確認/);assert.match(html,/手動実行チェックリスト/);assert.match(html,/計画識別子/);assert.match(html,/会計取引履歴/);assert.match(html,/Phase 8D-18/);
console.log('recovery funding execution readiness, finance marker, manual checklist, and stale-plan validation tests passed');