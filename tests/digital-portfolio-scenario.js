...
// The digital-business scale-up scenario shared by digital-portfolio-profitability-test.js and
// digital-portfolio-breakeven-seeds-test.js: a production engine played by a fixed policy.
...
const makeRandom=(start=SEED)=>{let seed=start;return()=>((seed=Math.imul(seed,1664525)+1013904223>>>0)/2**32);};
...
function run(strategy,maxWeeks=208,seed=SEED){
  const loaded=loadGame({random:makeRandom(seed),headless:true});
...
module.exports={SEED,CHECKPOINTS,makeRandom,run};
