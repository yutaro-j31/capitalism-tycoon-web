...
// 1. GP + LP拠出 → acquisition → exit のclaim reconciliation。
{
  ...
  const coinvestInvestment = pf.recordCoinvestment(e.g, fund, 1_000_000_000);
...
// 2. Annual management fee: fund cash → GP法人 company cash, once per anniversary.
{
  ...
  // T26-2: 報酬はGP法人へ入り、そこから投資チームの人件費が出る。size 10億×fee 2% = 年2億で
  // 養えるのは 2億/2,000万 = 10人。上限60人には当たらないので報酬はまるごと人件費に消える。
  const heads = pf.teamCapacity(fund);
  const payroll = heads * pf.MANAGEMENT_FEE_PER_HEAD;
  assert.equal(heads, 10, '年2億の管理報酬で養えるのは10人');
  pf.processFundsWeek(e.g, 53);
  assert.equal(fundBefore - fund.cash, 200_000_000, 'fund is the explicit fee payer');
  assert.equal(e.g.companyCash - companyBefore, 200_000_000 - payroll, 'GP法人 receives the fee and pays the team out of it');
  assert.equal(e.g.quarterlyPretaxProfit - pretaxBefore, 200_000_000 - payroll, 'fee is taxable GP法人 revenue, team payroll is its cost');
  assert.equal(fund.managementFeesPaid, 200_000_000);
  assert.equal(fund.teamPayrollPaid, payroll, 'team payroll is booked against the fund that pays for it');
  ...
  const payrollTx = e.g.finance.transactions.find(t => t.idempotencyKey === `pe-team-payroll-${fund.id}-p1`);
  assert.ok(payrollTx, 'GP法人の元帳にチーム人件費が記録される');
  assert.equal(payrollTx.cashEffect, -payroll);
  const snapshot = JSON.stringify({ fundCash: fund.cash, companyCash: e.g.companyCash, paid: fund.managementFeesPaid, payroll: fund.teamPayrollPaid, tx: e.g.finance.transactions.length });
  pf.processFundsWeek(e.g, 53);
  assert.equal(JSON.stringify({ fundCash: fund.cash, companyCash: e.g.companyCash, paid: fund.managementFeesPaid, payroll: fund.teamPayrollPaid, tx: e.g.finance.transactions.length }), snapshot, 'same period cannot pay twice');
