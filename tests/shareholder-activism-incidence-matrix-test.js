...
const { SCENARIOS, SEEDS, makeRandom, runScenario } = require('./strategy-balance-runner');
...
function openGrowthStore(engine, businessID, reserve) {
  ...
  return Boolean(engine.openStore({
    tenantID: tenant.id,
    businessID,
    name: `Incidence-${engine.g.stores.length + 1}`,
    operatingHours: 3
  }));
}

function applyStyle(engine, modules, style, postIpoWeek) {
  if (style === 'growth-reinvestment' && postIpoWeek % 8 === 0) {
    openGrowthStore(engine, 'ramen', 12_000_000);
  }
  ...
  // cash-hoarder deliberately makes no post-IPO capital-allocation action.
...
function runCase(style, seed) {
  ...
  for (let i = 0; i < remainingWeeks; i += 1) {
    ...
    if (!maxPressure || pressure.score > maxPressure.score) {
      maxPressure = {
        week: engine.g.week,
        score: Number(pressure.score.toFixed(4)),
        ...
        lowRoic: Number(pressure.metrics.lowRoic.toFixed(4)),
        governance: Number(pressure.metrics.governance.toFixed(4)),
        performance: Number(pressure.metrics.performance.toFixed(4)),
        cashHoardingBonus: pressure.metrics.cashHoardingBonus || 0
      };
    }
  ...
  assert(financeValidation.ok, JSON.stringify(financeValidation));
  ...
  return {
    ...
    campaignCount,
    maxPressure,
    endingCash: Math.round(engine.g.companyCash),
    retainedEarnings: Math.round(finance.balances?.retainedEarnings || 0),
    dividendPerShare: Number(engine.g.dividendPerShare || 0),
    stores: engine.g.stores.length,
    companyValue: Math.round(engine.companyValue()),
