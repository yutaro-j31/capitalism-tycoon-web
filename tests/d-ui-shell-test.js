...
// Old contract (kept here for history): mapEntities() could return up to 20
// entities, but the fixed-slot MARKER_POSITIONS table only had 10 base
// slots, so markerPosition()/markerPositionsCollide() existed to nudge
// colliding markers apart. Production promotion deleted the DOM-scraped
// mapEntities() adapter and this whole fixed-slot placement scheme along
// with it -- collision avoidance for the current, sole placement path
// (placeEntityTiles()'s deterministic tile assignment) is covered by
// tests/map-phase2-markers-test.js's "30 same-kind entities... all get
// unique tiles" and tests/map-phase2-production-promotion-test.js's
// no-exact-overlap checks instead.
