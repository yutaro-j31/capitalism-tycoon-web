...
assert.equal(p.eligible,true);assert.equal(p.primaryProceeds,37_200_000);assert.equal(p.secondaryProceeds,37_200_000);assert.equal(p.preparationCost,15_000_000);assert.equal(p.ownershipAfter,.818182);
assert.equal(engine.relistSubsidiary('sub-1','balanced'),true);
const sub=a.subsidiaries[0];assert.equal(a.companyCash,122_200_000);assert.equal(sub.subsidiaryCash,57_200_000);assert.equal(sub.valuation,437_200_000);assert.equal(sub.ownership,.818182);assert.equal(sub.publicCompany,true);assert.equal(sub.relistingRounds,1);assert.equal(sub.ipoOfferTerms.lockupUntilWeek,118);assert.equal(a.personalCash,beforePersonal);assert.equal(a.saveVersion,9);
assert.equal(events[0].type,'professionalServices');assert.equal(events[1].type,'investmentSale');assert.equal(events[1].meta.cashEffect,37_200_000);assert.ok(events[1].meta.assetEffect<0);
