...
game.g.finance.openingCash = game.g.companyCash;
game.g.finance.openingAssets = game.g.companyCash;
game.g.finance.openingEquity = game.g.companyCash;
game.g.finance.openingRetainedEarnings = game.g.companyCash;
game.g.week = 20;
...
const weeklyAIStates = weeklyGame.g.competitorStates;
const weeklyCounters = weeklyGame.seedCompetitorCounterStates();
const pressureCounter = weeklyCounters[0];
const pressureCompany = weeklyAIStates.find(row => row.competitorID === pressureCounter.competitorID);
pressureCounter.pricePressure = 3;
weeklyGame.__usingCompetitorCounterStates = true;
weeklyGame.g.competitorStates = weeklyCounters;
const pressureMultiplier = weeklyGame.competitorPressureMultiplier(pressureCounter.industryID, pressureCompany.marketPresence[0].prefID);
assert.ok(pressureMultiplier < 1, 'temporary counter view must apply saved price pressure');
assert.equal(weeklyGame.g.competitorStates.length, weeklyCounters.length, 'pressure calculation must not normalize away temporary counters');
assert.equal(weeklyGame.g.competitorStates[0].pricePressure, 3);
weeklyGame.g.competitorCounterStates = weeklyGame.g.competitorStates;
weeklyGame.g.competitorStates = weeklyAIStates;
weeklyGame.__usingCompetitorCounterStates = false;
competitor.ensureCounterStates(weeklyGame.g);
...
assert.equal(weeklyGame.g.competitorCounterStates.find(row => row.competitorID === pressureCounter.competitorID).pricePressure, 3, 'weekly parity processing must preserve counter pressure');
...
acquireGame.g.finance.openingCash = acquireGame.g.companyCash;
acquireGame.g.finance.openingAssets = acquireGame.g.companyCash;
acquireGame.g.finance.openingEquity = acquireGame.g.companyCash;
acquireGame.g.finance.openingRetainedEarnings = acquireGame.g.companyCash;
