...
{
 const e=engine();e.g.stores=[{status:'open',lastProfit:120_000},{status:'open',lastProfit:80_000},{status:'open',lastProfit:40_000}];assert.equal(mod.operatingRecoveryForWeek(e.g),true,'profitable multi-store operations are recovery evidence');e.g.stores[2].lastProfit=-300_000;e.g.weeklyProfitHistory=[250_000,120_000,-50_000,80_000];assert.equal(mod.operatingRecoveryForWeek(e.g),true,'positive trailing operating profit survives one loss week');assert.equal(mod.recoveryActionForWeek(e.g,99),true,'sustained operating recovery is recorded as a value-recovery action');e.g.weeklyProfitHistory=[100_000,-200_000,-150_000,-50_000];assert.equal(mod.operatingRecoveryForWeek(e.g),false,'sustained operating losses do not suppress value-destruction pressure');
}
{
