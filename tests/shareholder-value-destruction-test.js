...
{
 const e=engine();e.g.stores=[{status:'open',lastProfit:120_000},{status:'open',lastProfit:80_000},{status:'open',lastProfit:40_000}];assert.equal(mod.operatingRecoveryForWeek(e.g),true,'profitable multi-store operations are persistent recovery evidence');assert.equal(mod.recoveryActionForWeek(e.g,99),true,'operating recovery is recorded as a value-recovery action');e.g.stores[2].lastProfit=-300_000;assert.equal(mod.operatingRecoveryForWeek(e.g),false,'loss-making operations without a recovery posture do not suppress pressure');e.g.weeklyProfitHistory=[-100_000,250_000];assert.equal(mod.operatingRecoveryForWeek(e.g),true,'positive trailing operating profit is persistent recovery evidence');e.g.weeklyProfitHistory=[-100_000,-250_000];e.g.dividendPerShare=1;assert.equal(mod.operatingRecoveryForWeek(e.g),true,'an active payout policy is persistent recovery evidence for an operating company');e.g.dividendPerShare=0;assert.equal(mod.operatingRecoveryForWeek(e.g),false,'negative trailing performance without payout remains adverse');
}
{
