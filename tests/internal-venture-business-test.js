...
// A developing venture can be withdrawn with a deterministic recovery and explicit impairment accounting.
const withdrawalState=state(),withdrawalEngine=new Engine(withdrawalState),withdrawalVenture=withdrawalState.internalVentures[0],withdrawalPreview=mod.withdrawalPreview(withdrawalVenture),withdrawalCash=withdrawalState.companyCash,withdrawalPersonal=withdrawalState.personalCash;
assert.equal(withdrawalPreview.bookValue,20_000_000);assert.equal(withdrawalPreview.recoveryRate,.2764);assert.equal(withdrawalEngine.withdrawInternalVenture('iv-1'),true);assert.equal(withdrawalState.companyCash,withdrawalCash+withdrawalPreview.recoveredCash);assert.equal(withdrawalState.personalCash,withdrawalPersonal);assert.equal(withdrawalVenture.status,'withdrawn');assert.equal(withdrawalVenture.carryingBookValue,0);assert.equal(withdrawalVenture.withdrawalLoss,withdrawalPreview.impairment);assert.equal(withdrawalEngine.withdrawInternalVenture('iv-1'),false,'withdrawal cannot execute twice');
const withdrawalEvent=financeEvents.at(-1);assert.equal(withdrawalEvent.meta.operationID,'withdrawInternalVenture-iv-1-20');assert.deepEqual({cash:withdrawalEvent.meta.cashEffect,asset:withdrawalEvent.meta.assetEffect,profit:withdrawalEvent.meta.profitEffect},{cash:withdrawalPreview.recoveredCash,asset:-withdrawalPreview.bookValue,profit:-withdrawalPreview.impairment});
const withdrawalReload=JSON.parse(JSON.stringify(withdrawalState));mod.ensure(withdrawalReload);assert.equal(withdrawalReload.internalVentures[0].status,'withdrawn');assert.equal(mod.build(withdrawalReload).live,0);assert.match(mod.renderSection({g:withdrawalReload}),/撤退済み/);assert.match(mod.renderSection({g:state()}),/開発撤退/);
const deterministicWithdrawalA=state(),deterministicWithdrawalB=state();new Engine(deterministicWithdrawalA).withdrawInternalVenture('iv-1');new Engine(deterministicWithdrawalB).withdrawInternalVenture('iv-1');assert.deepEqual(JSON.parse(JSON.stringify(deterministicWithdrawalA)),JSON.parse(JSON.stringify(deterministicWithdrawalB)));
