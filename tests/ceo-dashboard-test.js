...
function mod(){const context={globalThis:{__capitalismTycoonModules:{}}};context.globalThis.globalThis=context.globalThis;vm.runInNewContext(code,context);return context.globalThis.__capitalismTycoonModules.ceoDashboard;}
...
const dashboard=mod();
const before=JSON.stringify(state());const a=dashboard.build(state(),{companyValue:30000000,ipoMissingReasons:[],missions:[{id:'m1',title:'創業'},{id:'m2',title:'上場準備'}]});const b=dashboard.build(state(),{companyValue:30000000,ipoMissingReasons:[],missions:[{id:'m1',title:'創業'},{id:'m2',title:'上場準備'}]});
...
const index=readIndex();assert.match(index,/js\/ceo-dashboard\.js/,'dashboard script is registered');
...
assert.ok(!forbidden.test(code),'dashboard module stays read-only and avoids forbidden mutations');
