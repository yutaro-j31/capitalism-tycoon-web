...
function mod(){const context={globalThis:{__capitalismTycoonModules:{executiveSecretary:{}}}};context.globalThis.globalThis=context.globalThis;vm.runInNewContext(code,context);return context.globalThis.__capitalismTycoonModules.ceoDashboard;}
...
const saveKey = 'capitalism_tycoon_web_v1';
const saveVersion = ctx.__capitalismTycoonModules.engine.SAVE_VERSION;
const snapshot = { json: JSON.stringify(engine.g), companyCash: engine.g.companyCash, companyDebt: engine.g.companyDebt, transactions: JSON.stringify(engine.g.finance?.transactions || []), weeklySnapshots: JSON.stringify(engine.g.finance?.weeklySnapshots || []), news: JSON.stringify(engine.g.news || []), selectedTab: engine.g.selectedTab, storage: JSON.stringify([...ctx.__localStorageData.entries()]), saveKey, saveVersion };
const valueBefore = engine.companyValue();
const ipoBefore = JSON.stringify(engine.ipoMissingReasons());
dashboard.build(engine.g,{companyValue:engine.companyValue(),ipoMissingReasons:engine.ipoMissingReasons(),missions:ctx.__capitalismTycoonModules.data.MISSION_DEFS});
const after = { json: JSON.stringify(engine.g), companyCash: engine.g.companyCash, companyDebt: engine.g.companyDebt, transactions: JSON.stringify(engine.g.finance?.transactions || []), weeklySnapshots: JSON.stringify(engine.g.finance?.weeklySnapshots || []), news: JSON.stringify(engine.g.news || []), selectedTab: engine.g.selectedTab, storage: JSON.stringify([...ctx.__localStorageData.entries()]), saveKey, saveVersion: ctx.__capitalismTycoonModules.engine.SAVE_VERSION };
assert.deepEqual(after, snapshot, 'dashboard accessors must not mutate engine state, save data, localStorage, SAVE_KEY, or saveVersion');
assert.equal(engine.companyValue(), valueBefore, 'companyValue accessor remains stable');
assert.equal(JSON.stringify(engine.ipoMissingReasons()), ipoBefore, 'ipoMissingReasons accessor remains stable');
