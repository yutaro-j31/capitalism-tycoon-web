...
// .d-city-blocks i::before/::after and the water/road-specific rules that used
// to live here (filter:drop-shadow, background-blend-mode:screen) were removed
// by production promotion along with the legacy .d-water/.d-road-grid/
// .d-city-blocks markup itself (see docs/map-phase2-production-integration-
// audit.md section 6, PR D) -- Phase 2's own Canvas draws that depth now.
// .d-city-surface::after (the renderer-agnostic overlay gradient, applies to
// the Phase 2 surface too) is the only depth treatment left here.
for(const mapDepthContract of ['.d-city-surface::after','pointer-events:none','radial-gradient(ellipse at 50% 42%','@media(max-width:820px)','@media(forced-colors:active)']){
  assert.ok(mapDepth.includes(mapDepthContract),`missing layered production-map depth treatment: ${mapDepthContract}`);
}
for(const removedMapDepthContract of ['.d-city-blocks i::before','.d-city-blocks i::after','.d-water','.d-road-grid']){
  assert.ok(!mapDepth.includes(removedMapDepthContract),`legacy map depth treatment must stay removed: ${removedMapDepthContract}`);
}
