...
f.rebuildDirtySnapshots(e.g); const st=f.buildStatements(e.g,'52'); if(st.workingCapital.accruedTaxes!==70) throw new Error('accrued tax mismatch'); if(st.cashFlow.accountsReceivableChange!==0) throw new Error('AR net change mismatch'); if(st.cashFlow.accruedTaxesChange!==70) throw new Error('tax WC change mismatch'); if(typeof st.workingCapital.workingCapitalChange!=='number') throw new Error('workingCapitalChange missing'); if(e.g.companyCash!==opening+1950) throw new Error('companyCash mismatch'); if(!f.validate(e.g).ok) throw new Error(f.validate(e.g).errors.join('\n'));
