...
for (const m of html.matchAll(/getElementById\(['"]([^'"]+)['"]\)|\$\(['"]#([^'"]+)['"]\)/g)) { const id = m[1] || m[2]; if (!ids.includes(id) && !/^screen|business-|overseas-business-|modal-|salary|so|ipo-|new-company|hire-ok|import-file|home-chart|report-chart/.test(id)) errors.push(`referenced id may be missing: ${id}`); }
