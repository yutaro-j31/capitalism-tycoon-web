...
const play=fs.readFileSync(path.join(__dirname,'../index.html'),'utf8'),runAll=fs.readFileSync(path.join(__dirname,'run-all.js'),'utf8');assert.match(play,/inter-subsidiary-synergy-performance\.js"/);assert.match(runAll,/inter-subsidiary-synergy-performance-test\.js/);assert.ok(!/Math\.random|Date\.now|personalCash\s*[+\-*/]?=|SAVE_KEY\s*=|saveVersion\s*=/.test(performance));
