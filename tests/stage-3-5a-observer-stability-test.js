...
assert.equal(comprehensive.length,32,`expected 32 residual observer-driven JS sources after deterministic-economic-foundation migration, got ${comprehensive.length}: ${comprehensive.join(', ')}`);
assert.equal(unqualified.length,32,`expected 32 residual unqualified observer-driven JS sources after deterministic-economic-foundation migration, got ${unqualified.length}`);
...
assert.equal(productionObserverSources.length,32,`repository-wide production scan expected 32 observer sources after deterministic-economic-foundation migration, got ${productionObserverSources.length}: ${productionObserverSources.map(row=>`${row.path}(${row.occurrences})`).join(', ')}`);
