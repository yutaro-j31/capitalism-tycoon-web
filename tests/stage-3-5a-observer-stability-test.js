...
assert.equal(comprehensive.length,0,`expected 0 residual observer-driven JS sources after deterministic-economic-foundation migration, got ${comprehensive.length}: ${comprehensive.join(', ')}`);
assert.equal(unqualified.length,0,`expected 0 residual unqualified observer-driven JS sources after deterministic-economic-foundation migration, got ${unqualified.length}`);
...
assert.equal(productionObserverSources.length,0,`repository-wide production scan expected 0 observer sources after deterministic-economic-foundation migration, got ${productionObserverSources.length}: ${productionObserverSources.map(row=>`${row.path}(${row.occurrences})`).join(', ')}`);
