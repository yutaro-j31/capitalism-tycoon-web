...
1. Strategic competitor memory and market-share response planning.
2. Player-versus-competitor share objectives, leader changes, rivalry intensity, and market defense.
3. Multi-business and multi-region competitor portfolio allocation.
4. Competitor product launches, R&D races, and imitation/counter-positioning.
...
Each unit must include focused tests, full regression checks, save compatibility, bounded state growth, finite-number validation, and a reviewable pull request. UI changes are excluded unless a gameplay system cannot be operated without a minimal interface addition.
