...
市場システム自体は決定論的だが、対象店舗の週次ループでは旧売上式の需要乱数スロットを消費する。消費値は売上に使わず、旧式の乱数呼び出し順を維持するためだけに存在する。

## 開店初週

今週 `openingWeek` に到達する店舗は市場計算前に `open` へ変更され、開店初週から市場結果を持つ。ニュースとstatus変更は1回だけ行う。
