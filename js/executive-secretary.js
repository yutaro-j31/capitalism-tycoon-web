...
function build(g,helpers={}){const stores=openStores(g);return Object.freeze({overview:Object.freeze({companyName:g?.companyName||'会社',years:Math.max(0,Math.floor((nf(g?.week,1)-1)/52)),cash:nf(g?.companyCash),companyValue:nf(helpers.companyValue),debt:nf(g?.companyDebt),stores:stores.length,employees:nf(g?.employeeCount,__modules.workforce?.companyHeadcount?.(g)??arr(g?.workforceTeams).reduce((a,t)=>a+nf(t?.headcount),0)),credit:nf(g?.companyCredit),ipo:g?.publicCompany?`上場 ${g?.ticker||''}`:'未上場'}),journey:companyJourney(g,helpers),finance:financialSummary(g,helpers),allocation:capitalAllocation(g,helpers),risks:risks(g,helpers),growth:growth(g,helpers),ma:maGovernance(g,helpers)});}
