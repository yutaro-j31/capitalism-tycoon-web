...
const BUSINESS_ID='realEstateAgency',SCHEMA_VERSION=1,COMMISSION_RATE=.03,HISTORY_LIMIT=52;
...
function processStore(g,store,business,pref){
  ...
  for(const deal of pipeline.activeDeals){
    const age=Math.max(0,week-deal.createdWeek),closeChance=clamp(.08+quality*.0022+dx*.001+(cycle-1)*.18+efficiency*.0006,.05,.58);
    ...
    if(age>=2&&closeRoll<closeChance){const negotiation=.88+hash(g.seed||1,`${deal.id}:value:${week}`)*.2,transactionValue=Math.round(deal.askingValue*negotiation);closedDeals++;closedTransactionVolume+=transactionValue;totalCloseWeeks+=age;}
  ...
  const available=Math.max(0,pipeline.capacity-pipeline.activeDeals.length),mandateChance=clamp(.18+brand*.002+quality*.0015+(cycle-1)*.12,.12,.58);let mandates=0;
...
modules.realEstateAgencyPipeline=Object.freeze({BUSINESS_ID,SCHEMA_VERSION,COMMISSION_RATE,HISTORY_LIMIT,marketIndicator,capacityFor,ensureStore,normalize,processStore});
})();
