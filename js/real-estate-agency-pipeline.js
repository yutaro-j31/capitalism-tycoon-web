...
const BUSINESS_ID='realEstateAgency',SCHEMA_VERSION=2,SINGLE_COMMISSION_RATE=.055,DOUBLE_COMMISSION_RATE=.065,DOUBLE_SIDE_RATE=.5,HISTORY_LIMIT=52;
...
function sanitizeDeal(deal,storeID,week,seed){
  ...
  return {id,storeID,createdWeek:Math.min(week,Math.max(1,integer(deal.createdWeek,week))),askingValue:Math.max(1_000_000,integer(deal.askingValue,30_000_000)),side};
}
...
function processStore(g,store,business,pref){
  ...
  let closedDeals=0,singleClosedDeals=0,doubleClosedDeals=0,lostDeals=0,closedTransactionVolume=0,singleCommissionRevenue=0,doubleCommissionRevenue=0,totalCloseWeeks=0;
  const survivors=[];
...
modules.realEstateAgencyPipeline=Object.freeze({BUSINESS_ID,SCHEMA_VERSION,SINGLE_COMMISSION_RATE,DOUBLE_COMMISSION_RATE,DOUBLE_SIDE_RATE,HISTORY_LIMIT,commissionRateForSide,sideForDeal,marketIndicator,capacityFor,eligibleStores,ensureStore,normalize,processStore});
})();
