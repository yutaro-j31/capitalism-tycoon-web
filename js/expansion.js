...
function installExpansion(TycoonEngine){
  ...
  TycoonEngine.prototype.updateCapitalMarketsExpandedWeekly=function(){
    const g=this.g;if(g.week%13===0){for(const s of g.market){const micro=globalThis.__capitalismTycoonModules?.microcapListings,revenue=s.marketCap*(s.microcap?.04+micro.unit('microcap-fundamentals',s.id,g.week,'revenue')*.08:rand(.04,.12)),profit=s.microcap?(s.microcapFundamentallyProfitable?revenue*(.03+micro.unit('microcap-fundamentals',s.id,g.week,'profit')*.21):0):revenue*rand(.03,.24),eps=profit/Math.max(1,s.issuedShares);g.quarterlyStockResults[s.id]=g.quarterlyStockResults[s.id]||[];g.quarterlyStockResults[s.id].unshift({week:g.week,revenue,profit,eps,price:s.price});g.quarterlyStockResults[s.id]=g.quarterlyStockResults[s.id].slice(0,12);}for(const st of g.startups){g.startupQuarterlyReports[st.id].unshift({week:g.week,valuation:st.valuation,runwayWeeks:st.runwayWeeks,growth:st.growth,risk:st.risk});g.startupQuarterlyReports[st.id]=g.startupQuarterlyReports[st.id].slice(0,12);}}
    for(const st of g.startups){const hist=g.startupFundingHistory[st.id];if(st.fundingOpen&&!hist.some(x=>x.openedWeek===g.week)){hist.unshift({id:uid(),stage:st.fundingRound||st.stage,openedWeek:g.week,valuation:st.valuation,status:'open'});g.startupFundingHistory[st.id]=hist.slice(0,24);}const latest=g.startupFundingHistory[st.id].find(x=>x.status==='open');if(latest&&!st.fundingOpen){latest.status='closed';latest.closedWeek=g.week;latest.postMoney=st.valuation;}}
    if(g.publicCompany){g.ownershipHistory.push({week:g.week,founder:g.founderOwnershipRatio,external:g.externalShareholderRatio,competitor:g.competitorOwnedRatio});g.ownershipHistory=g.ownershipHistory.slice(-260);if(g.founderOwnershipRatio<.5&&g.competitorOwnedRatio>.1&&!g.activistCampaigns.some(x=>x.status==='active')&&Math.random()<.04){const c={id:uid(),investorName:'アクティビスト・キャピタル',demand:pick(['不採算事業売却','増配','自社株買い','経営陣刷新']),createdWeek:g.week,expiresWeek:g.week+16,pressure:rand(.2,.7),status:'active'};g.activistCampaigns.unshift(c);g.shareholderEventLog.unshift(`第${g.week}週：${c.investorName}が「${c.demand}」を要求。`);g.shareholderEventLog=g.shareholderEventLog.slice(0,logCap('shareholderEventLog'));g.news.unshift(`第${g.week}週：アクティビスト株主が経営改善を要求しています。`);}}
  };
