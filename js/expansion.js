...
function installExpansion(TycoonEngine){
  ...
  TycoonEngine.prototype.sellPersonalRealEstate=function(id){const a=this.g.personalRealEstateHoldings.find(x=>x.assetID===id&&x.status==='owned');if(!a)return false;const mortgage=__modules.personalRealEstateMortgage,settlement=mortgage?mortgage.settleOnSale(this.g,a,a.currentValue):{repaid:0,shortfall:0,net:a.currentValue};this.g.personalCash+=settlement.net;a.status='sold';a.soldWeek=this.g.week;a.salePrice=a.currentValue;this.notify(settlement.repaid>0?`${a.name}を売却し、残債${Math.round(settlement.repaid).toLocaleString('ja-JP')}円を清算しました。`:`${a.name}を売却しました。`,'success');this.save();this.emit();return true;};
