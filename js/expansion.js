...
function installExpansion(TycoonEngine){
  ...
  TycoonEngine.prototype.updateCapitalMarketsExpandedWeekly=function(){
    const g=this.g;if(g.week%13===0){for(const s of g.market){const micro=globalThis.__capitalismTycoonModules?.microcapListings,revenue=s.marketCap*(s.microcap?.04+micro.unit('microcap-fundamentals',s.id,g.week,'revenue')*.08:rand(.04,.12)),profit=s.microcap?(s.per>0?revenue*(.03+micro.unit('microcap-fundamentals',s.id,g.week,'profit')*.21):0):revenue*rand(.03,.24),eps=profit/Math.max(1,s.issuedShares);g.quarterlyStockResults[s.id]=g.quarterlyStockResults[s.id]||[];g.quarterlyStockResults[s.id].unshift({week:g.week,revenue,profit,eps,price:s.price});g.quarterlyStockResults[s.id]=g.quarterlyStockResults[s.id].slice(0,12);}for(const st of g.startups){g.startupQuarterlyReports[st.id].unshift({week:g.week,valuation:st.valuation,runwayWeeks:st.runwayWeeks,growth:st.growth,risk:st.risk});g.startupQuarterlyReports[st.id]=g.startupQuarterlyReports[st.id].slice(0,12);}}
