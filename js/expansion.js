...
// Book-value depreciation, display-only: it never touches currentValue (the market-cycle
// figure that R4's earlier passes tuned and pinned -- see CLAUDE.md's "係数の調整は行わない"
// -- or personalCash/personalNetWorth/sale proceeds. It's the same non-cash, non-tax idea as
// the company real-estate depreciation (js/real-estate.js), reusing that module's exact
// usefulLifeWeeks/salvageRate constants for one shared schedule across both. Land has no
// equivalent here (personal holdings have no land/building split), so the whole purchase
// price depreciates.
...
// Simulation randomness and IDs come from the stream kept in the save (#731).
const simRng = __modules.simulationRng;
const rand = (g,min,max) => simRng.range(g,min,max);
const pick = (g,arr) => simRng.pick(g,arr);
const uid = (g,prefix='x') => simRng.nextID(g,prefix);
...
function installExpansion(TycoonEngine){
  ...
  TycoonEngine.prototype.founderHomeAction=function(action){
    ...
    if(action==='storeHunt'){
      ...
      const events=['商店街の空き店舗','地元銀行の相談会','友人の副業案件','先輩起業家の助言','地元メディア'];const e=pick(this.g,events);
  ...
  TycoonEngine.prototype.productFunnelAction=function(productID,action){
    ...
    if(action==='ads')f.awareness=clamp(f.awareness+(solo?.03:.045),0,1);if(action==='ux'){f.conversionModifier=n(f.conversionModifier)+(solo?.004:.006);f.churnModifier=n(f.churnModifier)-(solo?.002:.003);f.conversionRate=clamp(f.conversionRate+(solo?.004:.006),.003,.7);f.churnRate=clamp(f.churnRate-(solo?.002:.003),.003,.28);}if(action==='server'){p.serverCapacity=n(p.serverCapacity,5000)*1.45;f.serverLoad=clamp(f.serverLoad-.18,0,2);f.supportBurden=clamp(f.supportBurden-.03,0,1.5);}if(action==='b2b'){f.b2bContracts+=1;f.arpu*=1.015;}
  ...
  TycoonEngine.prototype.refreshInvestorOffers=function(){
    ...
    const count=Math.floor(rand(this.g,1,3));
    for(let i=0;i<count;i++){
      const preMoneyValuation=Math.round(baseValue*rand(this.g,.7,1.3));
      const equity=rand(this.g,.05,.2);
      const amount=Math.round(preMoneyValuation*equity/(1-equity));
      this.g.investorOffers.push({id:uid(this.g),name:pick(this.g,INVESTOR_NAMES),amount,equity,preMoneyValuation,expiresWeek:this.g.week+6,status:'pending'});
    }
  ...
  TycoonEngine.prototype.refreshStartupDealFlow=function(){
    ...
    const template=pick(this.g,candidates);
    const s={id:uid(this.g),name:template.name,domain:template.domain,stage:template.stage,
      valuation:Math.round(rand(this.g,template.valuationRange[0],template.valuationRange[1])),
      minTicket:Math.round(rand(this.g,template.minTicketRange[0],template.minTicketRange[1])),
      growth:rand(this.g,template.growthRange[0],template.growthRange[1]),
      risk:rand(this.g,template.riskRange[0],template.riskRange[1]),
      ...
      productProgress:rand(this.g,0,.15),runwayWeeks:Math.round(rand(this.g,40,60)),reports:[],fundingRound:template.stage,fundingOpen:true};
  ...
  TycoonEngine.prototype.createAngelInvestment=function(amount){amount=Math.max(500000,n(amount));if(this.g.personalCash<amount)return this.fail('個人資金が不足しています。');this.g.personalCash-=amount;this.g.angelInvestments.push({id:uid(this.g),startupName:`スタートアップ${this.g.week}-${Math.floor(rand(this.g,10,99))}`,investedAmount:amount,week:this.g.week,multiple:1,status:'active'});this.notify('エンジェル投資を実行しました。','success');this.save();this.emit();return true;};
  ...
  TycoonEngine.prototype.buyPersonalRealEstate=function(id){const o=PERSONAL_REAL_ESTATE_OFFERS.find(x=>x.id===id);if(!o||this.g.personalCash<o.price)return this.fail('個人資金が不足しています。');this.g.personalCash-=o.price;this.g.personalRealEstateHoldings.push({...copy(o),assetID:uid(this.g),purchasePrice:o.price,currentValue:o.price,bookValue:o.price,purchasedWeek:this.g.week,status:'owned',rentalOps:{occupancyStatus:'occupied',contractWeeklyRent:o.weeklyRent,leaseStartedWeek:this.g.week,leaseEndsWeek:this.g.week+PERSONAL_REAL_ESTATE_LEASE_WEEKS,renewalStatus:'none',renewalStrategyID:null,renewalProposedWeeklyRent:0,renewalDecisionChance:0,condition:90,weeksTracked:0,occupiedWeeks:0,vacantWeeks:0,currentVacancyWeeks:0,grossRentTotal:0,operatingExpenseTotal:0,repairExpenseTotal:0,noiTotal:0,lastGrossRent:0,lastOperatingExpense:0,lastNOI:0,lastProcessedWeek:null}});this.notify(`${o.name}を個人で購入しました。`,'success');this.save();this.emit();return true;};
  TycoonEngine.prototype.sellPersonalRealEstate=function(id){const a=this.g.personalRealEstateHoldings.find(x=>x.assetID===id&&x.status==='owned');if(!a)return false;if(__modules.personalRealEstateRedevelopment?.inProgress(a))return this.fail('開発・再開発が完了するまで売却できません。');const mortgage=__modules.personalRealEstateMortgage,mortgageSettlement=mortgage?mortgage.settleOnSale(this.g,a,a.currentValue):{repaid:0,shortfall:0,net:a.currentValue},taxes=__modules.personalRealEstateTaxes,taxSettlement=taxes?taxes.settleOnSale(this.g,a,mortgageSettlement.net):{paid:0,shortfall:0,net:mortgageSettlement.net};__modules.personalRealEstateCorp.creditOwnerAccount(this.g,a,taxSettlement.net);a.status='sold';a.soldWeek=this.g.week;a.salePrice=a.currentValue;const settlements=[];if(mortgageSettlement.repaid>0)settlements.push(`残債${Math.round(mortgageSettlement.repaid).toLocaleString('ja-JP')}円`);if(taxSettlement.paid>0)settlements.push(`固定資産税未納${Math.round(taxSettlement.paid).toLocaleString('ja-JP')}円`);this.notify(settlements.length?`${a.name}を売却し、${settlements.join('、')}を清算しました。`:`${a.name}を売却しました。`,'success');this.save();this.emit();return true;};
  TycoonEngine.prototype.getPersonalRealEstateOperations=function(id){const a=this.g.personalRealEstateHoldings.find(x=>x.assetID===id&&x.status==='owned');if(!a)return null;const ops=personalRealEstateOps(a,this.g.week),weeksUntil=Math.max(0,ops.leaseEndsWeek-this.g.week),price=Math.max(1,n(a.purchasePrice)),bookValue=Number.isFinite(a.bookValue)?a.bookValue:price;return {...ops,weeksUntilLeaseEnd:ops.occupancyStatus==='occupied'?weeksUntil:0,renewalAvailable:ops.occupancyStatus==='occupied'&&weeksUntil<=PERSONAL_REAL_ESTATE_RENEWAL_WINDOW&&weeksUntil>0&&ops.renewalStatus==='none',renewalStrategies:PERSONAL_REAL_ESTATE_RENEWAL_STRATEGIES.map(s=>({...s,proposedWeeklyRent:Math.round(ops.contractWeeklyRent*(1+s.rentChange))})),repairQuote:ops.condition<100?Math.max(50000,price*(100-ops.condition)*.0005):0,occupancyRate:ops.weeksTracked?clamp(ops.occupiedWeeks/ops.weeksTracked,0,1):(ops.occupancyStatus==='occupied'?1:0),grossYield:n(a.weeklyRent)*52/price,effectiveYield:ops.weeksTracked?ops.noiTotal/ops.weeksTracked*52/price:0,bookValue,weeklyDepreciation:personalRealEstateDepreciation({purchasePrice:price,bookValue})};};
  ...
  TycoonEngine.prototype.refreshSportsMarket=function(){
    const positions=['投手','捕手','内野手','外野手','FW','MF','DF','GK'];const surnames=['佐藤','鈴木','高橋','田中','山本','中村','小林','加藤'];this.g.sportsDraftCandidates=Array.from({length:6},()=>({id:uid(this.g),name:`${pick(this.g,surnames)} ${Math.floor(rand(this.g,10,99))}`,position:pick(this.g,positions),potential:rand(this.g,48,92),expectedSalary:rand(this.g,8000000,65000000)}));this.g.sportsTradeMarket=Array.from({length:6},()=>({id:uid(this.g),playerName:`${pick(this.g,surnames)} ${Math.floor(rand(this.g,10,99))}`,position:pick(this.g,positions),askingPrice:rand(this.g,20000000,180000000),rating:rand(this.g,52,90)}));this.g.lastSportsMarketWeek=this.g.week;this.save();this.emit();return true;
  };
  TycoonEngine.prototype.draftPlayer=function(teamID,candidateID){const t=this.g.sportsTeams.find(x=>x.id===teamID),c=this.g.sportsDraftCandidates.find(x=>x.id===candidateID);if(!t||!c)return false;const cashKey=t.owner==='company'?'companyCash':'personalCash';if(this.g[cashKey]<c.expectedSalary)return this.fail('契約資金が不足しています。');this.g[cashKey]-=c.expectedSalary;if(t.owner==='company')finance.event(this.g,'payroll',c.expectedSalary,{cashEffect:-c.expectedSalary,profitEffect:-c.expectedSalary,sourceType:'draftPlayer',sourceID:`${teamID}-${candidateID}`,description:'球団選手契約金'});t.teamStrength=clamp(t.teamStrength+c.potential/18,0,100);t.roster=(t.roster||[]);t.roster.push({...c,weeklySalary:__modules.sportsManagement?.salaryFromDraft(c)??0,signedWeek:this.g.week});this.g.sportsDraftCandidates=this.g.sportsDraftCandidates.filter(x=>x.id!==candidateID);this.notify(`${t.name}が${c.name}を指名しました。`,'success');this.save();this.emit();return true;};
  TycoonEngine.prototype.tradePlayer=function(teamID,assetID){const t=this.g.sportsTeams.find(x=>x.id===teamID),a=this.g.sportsTradeMarket.find(x=>x.id===assetID);if(!t||!a)return false;const cashKey=t.owner==='company'?'companyCash':'personalCash';if(this.g[cashKey]<a.askingPrice)return this.fail('獲得資金が不足しています。');this.g[cashKey]-=a.askingPrice;if(t.owner==='company')finance.event(this.g,'otherOperating',a.askingPrice,{cashEffect:-a.askingPrice,profitEffect:-a.askingPrice,sourceType:'tradePlayer',sourceID:`${teamID}-${assetID}`,description:'球団トレード獲得費'});t.teamStrength=clamp(t.teamStrength+a.rating/15,0,100);t.roster=(t.roster||[]);t.roster.push({...a,weeklySalary:__modules.sportsManagement?.salaryFromTrade(a)??0,signedWeek:this.g.week});this.g.sportsTradeMarket=this.g.sportsTradeMarket.filter(x=>x.id!==assetID);this.notify(`${t.name}が${a.playerName}をトレード獲得しました。`,'success');this.save();this.emit();return true;};
  TycoonEngine.prototype.signForeignPlayer=function(teamID){const t=this.g.sportsTeams.find(x=>x.id===teamID);if(!t)return false;const cost=rand(this.g,50000000,220000000),cashKey=t.owner==='company'?'companyCash':'personalCash';if(this.g[cashKey]<cost)return this.fail('補強資金が不足しています。');this.g[cashKey]-=cost;if(t.owner==='company')finance.event(this.g,'otherOperating',cost,{cashEffect:-cost,profitEffect:-cost,sourceType:'signForeignPlayer',sourceID:teamID,description:'球団外国人補強費'});t.teamStrength=clamp(t.teamStrength+rand(this.g,3,8),0,100);t.roster=(t.roster||[]);t.roster.push({id:uid(this.g),playerName:`外国人選手 ${Math.floor(cost/1000000)}`,position:'助っ人',rating:70,askingPrice:cost,weeklySalary:__modules.sportsManagement?.salaryFromForeign(cost)??0,signedWeek:this.g.week});this.notify(`${t.name}が外国人選手を補強しました。`,'success');this.save();this.emit();return true;};
  ...
  TycoonEngine.prototype.bidLuxuryAuction=function(id){const x=this.g.luxuryAuctionListings.find(x=>x.id===id&&x.status==='open');if(!x)return false;const price=x.currentBid*1.08;if(this.g.personalCash<price)return this.fail('個人資金が不足しています。');this.g.personalCash-=price;x.status='won';this.g.luxuryAssets.push({id:uid(this.g),name:x.name,category:x.category,purchasePrice:price,currentValue:price,maintenancePerWeek:price*.00015,rarity:x.rarity,statusEffect:'名声・人脈',purchasedWeek:this.g.week});this.g.personalFame+=x.rarity;this.notify(`${x.name}を落札しました。`,'success');this.save();this.emit();return true;};
  ...
  TycoonEngine.prototype.updateProductFunnelsWeekly=function(){
    ...
    for(const p of g.productVentures){const f=this.ensureProductFunnel(p);if(p.status!=='released')continue;const oldRevenue=n(p.revenue),oldCost=n(p.cost),solo=p.origin==='founderHome';if(!p.releaseWeek)p.releaseWeek=g.week;
      ...
      const delta=(revenue-cost)-(oldRevenue-oldCost);g.companyCash+=delta;adjustment+=delta;salesAdjustment+=revenue-oldRevenue;expenseAdjustment+=cost-oldCost;if(f.serverLoad>1.15&&simRng.next(this.g)<.08){f.churnRate=clamp(f.churnRate+.004,.003,.28);const text=`${p.name}のサーバー負荷が高く、解約率が悪化。`;g.productFunnelEventLog.unshift(`第${g.week}週：${text}`);g.news.unshift(`第${g.week}週：${text}`);}}
  ...
  TycoonEngine.prototype.updatePersonalExpandedWeekly=function(){
    ...
    for(const d of g.peDeals.filter(x=>x.status==='active')){d.holdingWeeks++;d.currentValuation=Math.max(d.investedAmount*.25,d.currentValuation*(1+rand(this.g,-.012,.025)+d.improvementScore/50000));
    ...
    for(const a of g.angelInvestments.filter(x=>x.status==='active')){const event=simRng.next(this.g);a.multiple=clamp(a.multiple*(event<.02?1.9:event>.98?.55:rand(this.g,.98,1.03)),.1,20);}
    for(const x of g.personalRealEstateHoldings.filter(x=>x.status==='owned')){const ops=personalRealEstateOps(x,g.week);if(ops.lastProcessedWeek===g.week)continue;if(!Number.isFinite(x.bookValue))x.bookValue=n(x.purchasePrice);const depreciation=personalRealEstateDepreciation(x);if(depreciation>0)x.bookValue=Math.max(0,x.bookValue-depreciation);const cycle=clamp((g.realEstateCycle+g.economy)/2,.7,1.4);x.currentValue=clamp(x.currentValue*(1.0005+(cycle-1)*.05),x.purchasePrice*.5,x.purchasePrice*6);x.weeklyRent=clamp(x.weeklyRent*(1+(cycle-1)*.03),x.purchasePrice*.0003,x.purchasePrice*.004);const redevelopment=__modules.personalRealEstateRedevelopment;if(redevelopment?.inProgress(x)){x.rentalOps=ops;redevelopment.processWeek(this,x);ops.lastProcessedWeek=g.week;continue;}const shortTerm=__modules.personalRealEstateShortTerm;if(shortTerm?.isShortTerm(x)){x.rentalOps=ops;const noi=shortTerm.processWeek(this,x);if(noi!==null&&noi!==undefined)adjustment+=noi;continue;}let acquiredThisWeek=false;if(ops.occupancyStatus==='occupied'&&g.week>=ops.leaseEndsWeek){if(ops.renewalStatus==='none'){const strategy=PERSONAL_REAL_ESTATE_RENEWAL_STRATEGIES[1],decision=this.personalRealEstateRenewalChance(x,ops,strategy);ops.renewalStatus=deterministicUnit(x.assetID,ops.leaseEndsWeek,strategy.id,'renewal')<decision.chance?'accepted':'declined';ops.renewalStrategyID=strategy.id;ops.renewalProposedWeeklyRent=decision.proposed;ops.renewalDecisionChance=decision.chance;}if(ops.renewalStatus==='accepted'){ops.contractWeeklyRent=ops.renewalProposedWeeklyRent;ops.leaseStartedWeek=g.week;ops.leaseEndsWeek=g.week+PERSONAL_REAL_ESTATE_LEASE_WEEKS;ops.renewalStatus='none';ops.renewalStrategyID=null;ops.renewalProposedWeeklyRent=0;ops.renewalDecisionChance=0;}else{ops.occupancyStatus='vacant';ops.contractWeeklyRent=0;ops.currentVacancyWeeks=0;}}
  ...
  TycoonEngine.prototype.updateSportsExpandedWeekly=function(){
    const g=this.g;if(g.sportsTeams.length&&!g.sportsDraftCandidates.length)this.refreshSportsMarket();if(g.week-g.lastSportsMarketWeek>=13)this.refreshSportsMarket();for(const t of g.sportsTeams){t.roster=t.roster||[];t.teamStrength=clamp(t.teamStrength+rand(this.g,-.25,.35),10,100);if(t.saleListed&&!g.sportsSaleOffers.some(x=>x.teamID===t.id&&x.status==='pending')&&simRng.next(this.g)<.08)g.sportsSaleOffers.push({id:uid(this.g),teamID:t.id,buyer:`投資家グループ${Math.floor(rand(this.g,10,99))}`,price:t.value*rand(this.g,.95,1.35),status:'pending',expiresWeek:g.week+8});}g.sportsSaleOffers.forEach(x=>{if(x.status==='pending'&&x.expiresWeek<g.week)x.status='expired';});
  };

  TycoonEngine.prototype.generateMediaWeekly=function(){
    const g=this.g;if(g.week-g.lastNewspaperWeek>=4){g.lastNewspaperWeek=g.week;const r=g.lastReport||{};const articles=[{category:'企業',title:`${g.companyName}、第${g.week}週の利益${Math.round(n(r.profit)).toLocaleString()}円`,detail:`直営${g.stores.length}店、企業価値${Math.round(this.companyValue()).toLocaleString()}円。`},{category:'市場',title:`景気指数${g.economy.toFixed(2)}、政策金利${(g.policyRate*100).toFixed(2)}%`,detail:g.macroCrisis?`${g.macroCrisis.kind}が継続中。`:'大きなマクロ危機は確認されていない。'},{category:'競合',title:'競争環境アップデート',detail:g.competitorEvents[0]||'主要競合は通常運転。'}];g.weeklyNewspaper.unshift({id:uid(this.g),week:g.week,name:`TYCOON WEEKLY 第${g.week}号`,articles});g.weeklyNewspaper=g.weeklyNewspaper.slice(0,52);}
    if(simRng.next(this.g)<.035){const item={id:uid(this.g),week:g.week,title:pick(this.g,['AI投資が加速','物流再編の波','消費者の節約志向','M&A市場が活況','人材獲得競争が激化']),impact:rand(this.g,-.04,.06),detail:'市場環境と企業戦略に影響する大型ニュース。'};g.majorBusinessNews.unshift(item);g.majorBusinessNews=g.majorBusinessNews.slice(0,80);g.economy=clamp(g.economy+item.impact,.72,1.28);g.news.unshift(`第${g.week}週：大型ニュース「${item.title}」`);}
    if(g.ventureForumEvents.filter(x=>x.status==='open'&&x.expiresWeek>=g.week).length<2&&simRng.next(this.g)<.08){g.ventureForumEvents.unshift({id:uid(this.g),name:pick(this.g,['東京スタートアップフォーラム','地域金融イノベーション会議','SaaS経営者サミット','グローバルVCデモデー']),fee:rand(this.g,100000,800000),networkGain:rand(this.g,2,6),repGain:rand(this.g,.5,2),status:'open',expiresWeek:g.week+12});g.ventureForumEvents=g.ventureForumEvents.slice(0,logCap('ventureForumEvents'));}g.ventureForumEvents.forEach(x=>{if(x.status==='open'&&x.expiresWeek<g.week)x.status='expired';});
    if(g.week-g.lastAuctionWeek>=8){g.lastAuctionWeek=g.week;g.luxuryAuctionListings=simRng.shuffle(this.g,LUXURY_AUCTION_POOL).slice(0,3).map(x=>({id:uid(this.g),...copy(x),currentBid:x.basePrice*rand(this.g,.8,1.25),status:'open',expiresWeek:g.week+8}));}
  };

  TycoonEngine.prototype.updateCapitalMarketsExpandedWeekly=function(){
    const g=this.g;if(g.week%13===0){for(const s of g.market){const micro=globalThis.__capitalismTycoonModules?.microcapListings,revenue=s.marketCap*(s.microcap?.04+micro.unit('microcap-fundamentals',s.id,g.week,'revenue')*.08:rand(this.g,.04,.12)),profit=s.microcap?(s.microcapFundamentallyProfitable?revenue*(.03+micro.unit('microcap-fundamentals',s.id,g.week,'profit')*.21):0):revenue*rand(this.g,.03,.24),eps=profit/Math.max(1,s.issuedShares);g.quarterlyStockResults[s.id]=g.quarterlyStockResults[s.id]||[];g.quarterlyStockResults[s.id].unshift({week:g.week,revenue,profit,eps,price:s.price});g.quarterlyStockResults[s.id]=g.quarterlyStockResults[s.id].slice(0,12);}for(const st of g.startups){g.startupQuarterlyReports[st.id].unshift({week:g.week,valuation:st.valuation,runwayWeeks:st.runwayWeeks,growth:st.growth,risk:st.risk});g.startupQuarterlyReports[st.id]=g.startupQuarterlyReports[st.id].slice(0,12);}}
    for(const st of g.startups){const hist=g.startupFundingHistory[st.id];if(st.fundingOpen&&!hist.some(x=>x.openedWeek===g.week)){hist.unshift({id:uid(this.g),stage:st.fundingRound||st.stage,openedWeek:g.week,valuation:st.valuation,status:'open'});g.startupFundingHistory[st.id]=hist.slice(0,24);}const latest=g.startupFundingHistory[st.id].find(x=>x.status==='open');if(latest&&!st.fundingOpen){latest.status='closed';latest.closedWeek=g.week;latest.postMoney=st.valuation;}}
    if(g.publicCompany){g.ownershipHistory.push({week:g.week,founder:g.founderOwnershipRatio,external:g.externalShareholderRatio,competitor:g.competitorOwnedRatio});g.ownershipHistory=g.ownershipHistory.slice(-260);if(g.founderOwnershipRatio<.5&&g.competitorOwnedRatio>.1&&!g.activistCampaigns.some(x=>x.status==='active')&&simRng.next(this.g)<.04){const c={id:uid(this.g),investorName:'アクティビスト・キャピタル',demand:pick(this.g,['不採算事業売却','増配','自社株買い','経営陣刷新']),createdWeek:g.week,expiresWeek:g.week+16,pressure:rand(this.g,.2,.7),status:'active'};g.activistCampaigns.unshift(c);g.shareholderEventLog.unshift(`第${g.week}週：${c.investorName}が「${c.demand}」を要求。`);g.shareholderEventLog=g.shareholderEventLog.slice(0,logCap('shareholderEventLog'));g.news.unshift(`第${g.week}週：アクティビスト株主が経営改善を要求しています。`);}}
  };
