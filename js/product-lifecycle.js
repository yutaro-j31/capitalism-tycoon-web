// Phase 8A-5: product lifecycle, technical debt, maintenance, sunsetting, and retirement.
(function(){'use strict';
...
const VERSION=2,HISTORY_LIMIT=80,POLICIES=Object.freeze({
...
function handleClick(e){const t=e?.target?.closest?.('[data-product-lifecycle-action]'),engine=modules.playerEngineBridge?.getEngine?.();if(!t||!engine)return false;e.preventDefault?.();const action=String(t.dataset.productLifecycleAction||''),id=String(t.dataset.productId||'');let ok=false;if(action==='policy')ok=engine.setProductMaintenancePolicy(id,String(t.dataset.policyId||''));else if(action==='sunset')ok=engine.startProductSunset(id);else if(action==='cancel-sunset')ok=engine.cancelProductSunset(id);else if(action==='retire')ok=engine.retireProduct(id);if(ok)enhance();return Boolean(ok);}
...
install();installUI();modules.productLifecycle=Object.freeze({VERSION,POLICIES,STAGES,ensure,weeklyCost,riskBand,validate,renderSection,enhance,__installed:true});
})();