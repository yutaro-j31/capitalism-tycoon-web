...
function selectedDetail(entity,g){
  ...
  if(entity.kind==='tenant'){
    ...
    return `<div class="d-context-hero"><div class="d-store-visual tenant"><span>${occupied?'OCCUPIED':'FOR LEASE'}</span></div><div class="d-rating"><b>${occupied?'入居中':'テナント募集'}</b><span>${status}</span></div></div><h3 class="d-context-name">${esc(entity.name)}</h3><div class="d-context-metrics"><div><span>募集週額賃料</span><strong>${money(tenant.rent)}</strong></div><div><span>契約保証金（閉店時返還なし）</span><strong>${money(tenant.deposit)}</strong></div><div><span>都道府県</span><strong>${prefLabel(tenant.prefID||entity.pref)}</strong></div><div><span>商圏</span><strong>${esc(tenant.cityName||'—')}</strong></div><div><span>物件交通量</span><strong>${tenant.traffic?finite(tenant.traffic).toFixed(2):'—'}（${suitability.trafficLabel}）</strong></div><div><span>区画規模</span><strong>${esc(tenant.size||'—')}</strong></div><div><span>業態適性</span><strong>${suitability.grade}（需要補正 ${impact}）</strong></div><div><span>適性理由</span><strong>${esc(suitability.reasons.join(' / '))}</strong></div><div><span>状態</span><strong>${status}</strong></div></div>${action}`;
  }
