...
const EASY_EXPANSION_RESERVE=6000000,HARD_STARTUP_CREDIT_FLOOR=5000000,STARTUP_GUARANTEE_END_WEEK=26;
...
function easyExpansionReserve(state){return difficultyProfile(state?.difficulty).id==='easy'&&Array.isArray(state?.stores)&&state.stores.length>0?Math.max(EASY_EXPANSION_RESERVE,Math.max(0,finite(state?.lastReport?.expenses))*4):0;}
function hardStartupCreditFloor(state){return difficultyProfile(state?.difficulty).id==='hard'&&Array.isArray(state?.stores)&&state.stores.length===0&&integer(state?.week,1)<=STARTUP_GUARANTEE_END_WEEK?HARD_STARTUP_CREDIT_FLOOR:0;}
function storeOpeningCost(tenant,business){return Math.max(0,finite(business?.storeCost)+finite(tenant?.deposit));}
function canOpenStoreSafely(state,tenant,business){const reserve=easyExpansionReserve(state),cost=storeOpeningCost(tenant,business);return Object.freeze({allowed:reserve===0||finite(state?.companyCash)-cost>=reserve,cost,reserve,cashAfter:finite(state?.companyCash)-cost});}
...
const baseCompanyCreditLimit=EngineClass.prototype.companyCreditLimit;EngineClass.prototype.companyCreditLimit=function(){return Math.max(baseCompanyCreditLimit.call(this),hardStartupCreditFloor(this.g));};
const baseOpenStore=EngineClass.prototype.openStore;EngineClass.prototype.openStore=function(options={}){const tenant=this.g.tenants.find(row=>row.id===options.tenantID),business=this.business(options.businessID),safety=canOpenStoreSafely(this.g,tenant,business);if(tenant&&business&&!safety.allowed)return this.fail(`やさしいモードでは出店後に安全運転資金${Math.round(safety.reserve).toLocaleString('ja-JP')}円を残してください。`);return baseOpenStore.call(this,options);};
...
modules.difficultyScenarioBalance=Object.freeze({VERSION,STANDARD_TARGET_WEEK,HISTORY_LIMIT,WARNING_WEEKS,EASY_EXPANSION_RESERVE,HARD_STARTUP_CREDIT_FLOOR,STARTUP_GUARANTEE_END_WEEK,DIFFICULTY_PROFILES,SCENARIO_PROFILES,difficultyProfile,scenarioProfile,gradeForWeek,scoreForWeek,easyExpansionReserve,hardStartupCreditFloor,storeOpeningCost,canOpenStoreSafely,ensure,reconcileOpeningFinance,evaluate,snapshot,validate,__installed:true});
})();
