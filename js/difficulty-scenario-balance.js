...
const baseConfigure=EngineClass.prototype.configure;EngineClass.prototype.configure=function(options={}){return this.runTransaction(()=>{const result=baseConfigure.call(this,options);reconcileOpeningFinance(this.g);applyEasyDemand(this.g);evaluate(this.g);return result;});};
...
const activeEngine=modules.playerEngineBridge.getEngine();if(activeEngine){const changed=reconcileOpeningFinance(activeEngine.g)|applyEasyDemand(activeEngine.g);ensure(activeEngine.g);if(changed)activeEngine.save();}
