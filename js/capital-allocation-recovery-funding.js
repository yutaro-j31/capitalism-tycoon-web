...
const stress=modules.capitalAllocationStressTest;
const policy=modules.capitalAllocationPolicy;
const DEFAULT_TARGETS=Object.freeze([50,60,70,80]);
const STOCK_FEE_RATE=.001;
const PROPERTY_SALE_RATE=.97;
const COMPOUND_SCENARIO=stress.SCENARIOS.find(row=>row.id==='compoundShock')||stress.SCENARIOS.at(-1);
...
function scoreForFunding(metrics,policyId,assetCash=0,borrowing=0){
 ...
 const funded=stress.recalculate(metrics,{cash:Math.max(0,finite(metrics?.cash)+cash+debtFunding),debt:Math.max(0,finite(metrics?.debt)+debtFunding)});
 const stressed=stress.applyScenario(funded,COMPOUND_SCENARIO);
...
function sourceInventory(instance){
 ...
 for(const [stockId,holding] of Object.entries(state.companyStocks||{})){
  const stock=marketById.get(stockId),units=whole(holding?.qty),price=Math.max(0,finite(stock?.price));
  if(!stock||units<1||price<=0)continue;
  const averageCost=Math.max(0,finite(holding?.avg,price)),marketValue=price*units,carryingValue=averageCost*units,unitProceeds=price*(1-STOCK_FEE_RATE),proceeds=whole(unitProceeds*units);
 ...
 for(const property of state.properties||[]){
  ...
  const marketValue=Math.max(0,finite(property?.value,finite(property?.price))),carryingValue=Math.max(0,finite(property?.bookValue,finite(property?.purchasePrice,marketValue))),proceeds=whole(marketValue*PROPERTY_SALE_RATE);
  if(proceeds<=0)continue;
 ...
 const companyValue=Math.max(0,finite(instance?.companyValue?.(),finite(state.companyCash)-finite(state.companyDebt)));
 const currentDebt=Math.max(0,finite(state.companyDebt));
 const creditLimit=Math.max(0,finite(instance?.companyCreditLimit?.(),0));
 const creditFactor=companyValue>0?creditLimit/companyValue:0;
 const debtCapacity=Math.max(0,whole(creditLimit-currentDebt));
 const excludedProducts=(state.productVentures||[]).filter(row=>row&&row.status!=='sold').length;
 const excludedBusinesses=(state.businesses||[]).filter(row=>row&&row.active!==false).length;
 return{week:Math.max(1,whole(state.week)||1),companyValue,currentDebt,creditLimit,creditFactor,debtCapacity,totals,assetCapacity:totals.securities+totals.properties,sources,excludedOperatingAssets:{products:excludedProducts,businesses:excludedBusinesses,total:excludedProducts+excludedBusinesses}};
}
...
function allocateSources(sources,desiredAmount,maxPriority=2){
 ...
 for(const source of Array.isArray(sources)?sources:[]){
  ...
  if(!source.discrete&&remaining<source.proceeds){
   units=Math.max(1,Math.min(source.units,Math.ceil(remaining/Math.max(.000001,source.unitProceeds))));
   selectedProceeds=whole(units*source.unitProceeds);
   selectedMarketValue=units*source.unitMarketValue;
   selectedCarryingValue=units*source.unitCarryingValue;
  }
...
function debtCapacityAfter(inventory,allocation){
 const projectedCompanyValue=Math.max(0,inventory.companyValue+allocation.marketValueChange);
 const creditLimit=Math.max(0,projectedCompanyValue*inventory.creditFactor);
 return{projectedCompanyValue,creditLimit,capacity:Math.max(0,whole(creditLimit-inventory.currentDebt))};
}
function minimumCashOnly(metrics,policyId,targetScore){
 const target=Math.round(clamp(targetScore,0,100)),base=scoreForFunding(metrics,policyId,0,0);
 if(base.score>=target)return{reachable:true,amount:0,baseScore:base.score,projectedScore:base.score};
 ...
 const final=scoreForFunding(metrics,policyId,amount,0);
 return{reachable:final.score>=target,amount,baseScore:base.score,projectedScore:final.score};
}
function minimumBorrowing(metrics,policyId,targetScore,assetCash,capacity){
 ...
 let previousAmount=0,low=null,high=null,bestAmount=0,maxScore=base.score;
 const segments=256;
 for(let i=1;i<=segments;i++){
  ...
  if(next.score>=target){low=previousAmount;high=amount;break;}
  previousAmount=amount;if(amount>=maxAmount)break;
 }
 ...
 const final=scoreForFunding(metrics,policyId,assetCash,amount);
 return{reachable:final.score>=target,amount,bestAmount:amount,baseScore:base.score,projectedScore:final.score,maxScore:Math.max(maxScore,final.score),scoreAtCapacity:atCapacity.score};
}
function summarizePlan(target,policyRow,recovery,inventory,metrics,allocation,borrowingAnalysis,status,reason){
 const borrowing=borrowingAnalysis?.reachable?Math.max(0,finite(borrowingAnalysis.amount)):Math.max(0,finite(borrowingAnalysis?.bestAmount));
 const projection=scoreForFunding(metrics,policyRow.id,allocation.proceeds,borrowing),targetReached=projection.score>=target;
 const planned={securities:allocation.byKind.securities,properties:allocation.byKind.properties,borrowing};
 const sequence=[...allocation.selected.map(row=>({id:row.id,kind:row.kind,name:row.name,amount:row.selectedProceeds,units:row.selectedUnits,discrete:row.discrete})),...(borrowing>0?[{id:'borrowing',kind:'borrowing',name:'会社借入',amount:borrowing,units:1,discrete:false}]:[])];
 const creditAfterDisposals=debtCapacityAfter(inventory,allocation);
...
function fundingPlan(instance,targetScore=70){
 ...
 for(const property of properties){
  ...
  if(projection.score>=target){const result=summarizePlan(target,policyRow,recovery,inventory,metrics,allocation,lastBorrowing,'strategicAssetRecovery','会社保有株式に加えて会社不動産の売却が必要です。');return{...result,cashOnlyNeed:cashNeed};}
 ...
 const result=summarizePlan(target,policyRow,recovery,inventory,metrics,allocation,lastBorrowing,'unreachable','非事業資産と有効な借入余力を用いても目標へ到達できません。');
 return{...result,cashOnlyNeed:cashNeed,targetReached:false};
}
function fundingFrontier(instance,targets=DEFAULT_TARGETS){
 const normalized=stress.normalizeTargets(targets),plans=normalized.map(target=>fundingPlan(instance,target));
 const rows=plans.map(plan=>({targetScore:plan.targetScore,status:plan.status,targetReached:plan.targetReached,targetPolicy:plan.targetPolicy,targetPolicyName:plan.targetPolicyName,projectedScore:plan.projectedScore,totalFunding:plan.totalFunding,assetFunding:plan.assetFunding,borrowing:plan.borrowing,policyWaitWeeks:plan.policyWaitWeeks,sequenceLength:plan.sequence.length}));
 return{week:plans[0]?.week??Math.max(1,whole(instance?.g?.week)||1),targets:normalized,rows,plans};
}
...
function render(instance=modules.playerEngineBridge?.getEngine?.()){
 ...
 const sequence=plan.sequence.length?plan.sequence.map((row,index)=>`${index+1}. ${row.name} ${compactYen(row.amount)}`).join(' → '):'追加調達なし';
 const frontierRows=frontier.rows.map(row=>`<div class="stat"><span>目標 ${row.targetScore}点</span><strong>${row.targetReached?compactYen(row.totalFunding):'到達不能'}</strong><small>${esc(row.targetPolicyName||'方針なし')}・資産 ${compactYen(row.assetFunding)}・借入 ${compactYen(row.borrowing)}</small></div>`).join('');
 const need=plan.cashOnlyNeed?.reachable?compactYen(plan.cashOnlyNeed.amount):'現金のみでは到達不能',gainLoss=plan.disposalGainLoss>=0?`売却益 ${compactYen(plan.disposalGainLoss)}`:`売却損 ${compactYen(Math.abs(plan.disposalGainLoss))}`;
