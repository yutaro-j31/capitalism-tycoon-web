...
// Phase 8A.2: each strategy now also carries the market share it is actually trying to hold.
// Before this, a competitor only had reactive thresholds (entry/exit) -- it never had a position
// it was aiming for, so it could not tell "we are winning" from "we are losing" and behaved the
// same either way. targetShare gives every competitor a persistent goal that its 13-week share
// memory is measured against, and the gap to that goal modulates the existing price and
// investment decisions (see shareAmbition/decide below).
const STRATEGIES=Object.freeze({
 low_price:{id:'low_price',name:'低価格型',description:'低価格と稼働率を重視し、余剰能力を価格で埋める。',targetPriceIndex:.92,priceAggressiveness:.85,qualityPriority:.35,brandPriority:.35,marketingPriority:.45,capacityPriority:.55,cashBufferWeeks:6,debtTolerance:.45,entryThreshold:.62,exitThreshold:.22,turnaroundBias:.65,riskTolerance:.62,targetShare:.24},
 quality:{id:'quality',name:'品質型',description:'品質と満足度を重視し、高単価・高原価になりやすい。',targetPriceIndex:1.10,priceAggressiveness:.35,qualityPriority:.90,brandPriority:.55,marketingPriority:.50,capacityPriority:.45,cashBufferWeeks:10,debtTolerance:.35,entryThreshold:.70,exitThreshold:.18,turnaroundBias:.50,riskTolerance:.42,targetShare:.12},
 brand:{id:'brand',name:'ブランド型',description:'広告と信頼を重視し、ブランド投資が厚い。',targetPriceIndex:1.12,priceAggressiveness:.42,qualityPriority:.55,brandPriority:.90,marketingPriority:.90,capacityPriority:.42,cashBufferWeeks:9,debtTolerance:.40,entryThreshold:.67,exitThreshold:.20,turnaroundBias:.48,riskTolerance:.50,targetShare:.16},
 convenience:{id:'convenience',name:'利便性型',description:'店舗網と能力を重視し、利便性で需要を獲得する。',targetPriceIndex:1.02,priceAggressiveness:.48,qualityPriority:.45,brandPriority:.45,marketingPriority:.55,capacityPriority:.92,cashBufferWeeks:8,debtTolerance:.55,entryThreshold:.58,exitThreshold:.16,turnaroundBias:.55,riskTolerance:.66,targetShare:.22},
 balanced:{id:'balanced',name:'バランス型',description:'価格・品質・ブランド・店舗網を均衡させる。',targetPriceIndex:1.00,priceAggressiveness:.55,qualityPriority:.58,brandPriority:.58,marketingPriority:.58,capacityPriority:.58,cashBufferWeeks:8,debtTolerance:.38,entryThreshold:.65,exitThreshold:.18,turnaroundBias:.52,riskTolerance:.50,targetShare:.18}
});
// How hard the share gap pulls on the existing decisions. Deliberately small: the strategy's own
// targetPriceIndex stays the dominant term, and the ambition only tilts it. Clamped so a wildly
// out-of-range share cannot invert a strategy's identity.
const SHARE_AMBITION_LIMIT=.5,SHARE_PRICE_SENSITIVITY=.35,SHARE_INVESTMENT_SENSITIVITY=40,SHARE_CAPACITY_SENSITIVITY=.15;
function strategyTargetShare(s){return clamp(finite(s&&s.targetShare,STRATEGIES.balanced.targetShare),0,1);}
// Positive when the competitor is BELOW the share it wants (push harder), negative when it is
// comfortably above (protect margin instead). Measured against the same 13-week presence history
// the rest of the decision logic already uses, so a single good or bad week cannot whipsaw it.
function shareAmbition(state,c,p,s){
 const rows=historyWindow(state.competitorPresenceHistoryByID,p.presenceID,13,state.week);
 const observed=rows.length?rows.reduce((a,x)=>a+clamp(x.marketShare,0,1),0)/rows.length:clamp(p.currentWeekShare,0,1);
 const target=clamp(finite(c.targetShare,strategyTargetShare(s)),0,1);
...
function decide(state,c){
 ...
 const ambition=shareAmbition(state,c,p,s);
 const reasons=[{code:'share',text:`市場シェア ${(p.currentWeekShare*100).toFixed(1)}%`},{code:'targetShare',text:`目標シェア ${(clamp(finite(c.targetShare,strategyTargetShare(s)),0,1)*100).toFixed(1)}% に対し ${ambition>0?'不足':'超過'} ${(Math.abs(ambition)*100).toFixed(1)}pt`},{code:'util',text:`能力稼働率 ${(util*100).toFixed(0)}%`},{code:'margin',text:`限界利益率 ${(cm*100).toFixed(0)}%`},{code:'strategy',text:`${s.name}戦略`},{code:'portfolio',text:`全${(c.marketPresence||[]).filter(x=>x.active).length}市場を比較し ${p.prefID} を選択`}];
 if(state.week%4===0){let target=(avgPrice||920)*s.targetPriceIndex*(1-ambition*SHARE_PRICE_SENSITIVITY);if(c.status==='distressed'||cm<.18)target=Math.max(target,p.price*1.04);const diff=(target-p.price)/Math.max(1,p.price);if(Math.abs(diff)>.025){const pct=clamp(diff,-.08,.08)*s.priceAggressiveness;const next=Math.max(Math.round(c.baseUnitCost*1.08),Math.round(p.price*(1+pct)));addAction(state,c,p,pct>=0?'priceIncrease':'priceDecrease',next,0,reasons,1);return;}if(c.cash<(c.weeklyFixedCost+c.weeklyMarketingCost)*s.cashBufferWeeks&&c.debt<c.cash*s.debtTolerance+12000000){addAction(state,c,null,'borrow',Math.round((c.weeklyFixedCost+200000)*8),0,[{code:'cash',text:`現金余力 ${Math.floor(c.cash/Math.max(1,c.weeklyFixedCost+1))}週`},{code:'credit',text:`信用 ${c.creditScore.toFixed(0)}`}],1);return;}}
 if(state.week%13===0&&c.cash>800000){if(s.marketingPriority>.55&&p.brandAwareness<clamp(75+ambition*SHARE_INVESTMENT_SENSITIVITY,40,100)){addAction(state,c,p,'brandInvestment',clamp(p.brandAwareness+2+s.marketingPriority*2,0,100),300000*s.marketingPriority,reasons,1);return;}if(s.qualityPriority>.55&&p.quality<clamp(78+ambition*SHARE_INVESTMENT_SENSITIVITY,40,100)){addAction(state,c,p,'qualityInvestment',clamp(p.quality+2+s.qualityPriority*2,0,100),500000*s.qualityPriority,reasons,4);return;}if((util>clamp(.85-ambition*SHARE_CAPACITY_SENSITIVITY,.5,1)||p.lostDemand>20)&&s.capacityPriority>.5){addAction(state,c,p,'capacityExpansion',Math.round(p.totalCapacity*1.12),700000*s.capacityPriority,reasons,4);return;}if(c.lossWeeks>=8&&p.currentWeekShare<s.exitThreshold){addAction(state,c,p,'marketExit',0,250000,reasons,2);return;}}
...
Object.assign(exports,{TARGET_BUSINESS_ID:TARGET,STRATEGIES,MAX_HISTORY,SHARE_AMBITION_LIMIT,SHARE_PRICE_SENSITIVITY,strategyTargetShare,shareAmbition,strategyID,ensure,migrateV8,offers,receiveMarketResults,recordHistories,evaluatePresences,selectDecisionPresence,processWeek,validate});
