...
(function(exports,data,market,finance,supply){
...
class TycoonEngine extends EventTarget {
  ...
  normalize() {
    this.g.saveVersion = SAVE_VERSION; supply.ensure(this.g);
  ...
  establishDepartment(id) {
    ...
    this.g.companyCash-=d.setupCost;finance.event(this.g,'headOfficeExpense',d.setupCost,{cashEffect:-d.setupCost,profitEffect:-d.setupCost,sourceType:'establishDepartment',sourceID:id,description:`${d.name} 設置費`});this.g.departments[id]={...deepClone(d),established:true};this.g.departmentStaff[id]=1;
  ...
  hireDepartmentStaff(id,count=1) {
    ...
    this.g.companyCash-=cost;finance.event(this.g,'payroll',cost,{cashEffect:-cost,profitEffect:-cost,sourceType:'hireDepartmentStaff',sourceID:`${id}-${this.g.week}`,description:'採用費'});this.g.departmentStaff[id]=finite(this.g.departmentStaff[id])+count;
  ...
  advanceWeek(showSummary=true) {
    ...
    supply.ensure(this.g);const beginningCash=this.g.companyCash;this.updateMacro();this.updateMarket();this.updateProperties();this.updateStartups();this.updateCompetitors();this.updateDirectivesAndCampaigns();
    ...
    for(const store of this.g.stores){if(store.status==='preparing'&&this.g.week>=store.openingWeek){store.status='open';store.weeksToOpen=0;this.g.news.unshift(`第${this.g.week}週：${store.name}が開店しました。`);}if(store.status==='open'&&supply.isTargetBusinessID(store.businessID)){supply.initializeStoreInventory(this.g,store);const inv=this.g.inventoryByStoreID[store.id];if(inv&&!inv.accountedInitialValue){const v=Object.values(inv.materials||{}).reduce((a,x)=>a+(x.lots||[]).reduce((b,l)=>b+finite(l.quantity)*finite(l.unitCost),0),0);if(v>0){finance.event(this.g,'workingCapitalIncrease',v,{cashEffect:0,profitEffect:0,assetEffect:v,liabilityEffect:v,payableAmount:v,inventoryAmount:v,businessID:store.businessID,storeID:store.id,sourceType:'initial-inventory-v6',sourceID:store.id,idempotencyKey:`initial-inventory-v6-${store.id}`,operationID:`initial-inventory-v6-${store.id}`,description:'Phase 1C開始在庫と開始買掛金'});}inv.accountedInitialValue=true;}}}
    supply.spoil(this.g,finance);supply.receiveOrders(this.g,finance);supply.payables(this.g,finance);
    ...
    const financeStores=[];
    let sales=product.revenue+overseas.revenue+subs.revenue+franchise,expenses=product.cost+overseas.cost,supplyCogsNonCash=0,rentIncome=0,stockIncome=0,dividend=0,propertyDepreciation=0;
    for(const store of this.g.stores){if(store.status!=='open'){store.weeksToOpen=Math.max(0,store.openingWeek-this.g.week);continue;}
      ...
      let mr=marketBatch.byStore[store.id];mr=supply.applyConstraint(this.g,store,mr);marketBatch.byStore[store.id]=mr;fixed=(b.fixedCost+p.rent+b.wage)*this.g.inflation*([0,.55,.8,1,1.24][store.operatingHours||3]||1)*(this.g.macroCrisis?.costMultiplier||1);repair=Math.max(0,100-store.condition)*650;if(finite(mr.emergencyProcurementCost)>0)finance.event(this.g,'workingCapitalIncrease',mr.emergencyProcurementCost,{cashEffect:-mr.emergencyProcurementCost,profitEffect:0,assetEffect:mr.emergencyProcurementCost,inventoryAmount:mr.emergencyProcurementCost,businessID:store.businessID,storeID:store.id,sourceType:'emergency-procurement',sourceID:`${this.g.week}-${store.id}`,idempotencyKey:`emergency-${this.g.week}-${store.id}`,operationID:`emergency-${this.g.week}-${store.id}`,description:'緊急仕入'});storeSales=mr.revenue;variable=mr.variableCost;supplyCogsNonCash+=variable;store.marketResult={...mr};store.lastSales=storeSales;store.lastProfit=storeSales-variable-fixed-repair;}
    ...
    supply.autoOrder(this.g);const operatingProfit=sales+rentIncome+stockIncome-expenses+subs.profit;let tax=0;if(this.g.week%13===0&&operatingProfit>0){tax=operatingProfit*.306;expenses+=tax;}
    const profit=sales+rentIncome+stockIncome-expenses+subs.profit;this.g.companyCash+=profit+supplyCogsNonCash;this.g.companyCredit=clamp(this.g.companyCredit+(profit>=0?.15:-.3),0,100);this.g.companyReputation=clamp(this.g.companyReputation+(profit>0?.08:-.04),0,100);
...
})(__modules.engine={},__modules.data,__modules.market,__modules.finance,__modules.supply);
