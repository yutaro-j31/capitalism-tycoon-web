...
function storeMenuPlan(state,store,business){const menus=globalThis.__capitalismTycoonModules?.storeEquipment;return menus?.menuPlan(state,store,business)||null;}
function menuChoice(offer,seg){const items=offer.menuPlan?.items||[];if(items.length<=1)return null;const scored=items.map(item=>{const rel=(offer.price-item.effectivePrice)/Math.max(1,offer.price);return {item,score:rel*seg.priceSensitivity*2.2+item.qualityDelta/20*seg.qualitySensitivity+item.noveltyDelta/25*seg.noveltySensitivity+finite(item.segmentFit?.[seg.id])};});const max=Math.max(...scored.map(x=>x.score)),weights=scored.map(x=>Math.exp(clamp(x.score-max,-50,50))),den=sum(weights);return scored.map((x,i)=>({item:x.item,share:weights[i]/den}));}
function menuPortfolio(offer,seg){const choice=menuChoice(offer,seg);if(!choice)return null;return {choice,price:sum(choice.map(x=>x.share*x.item.effectivePrice)),quality:sum(choice.map(x=>x.share*x.item.qualityDelta)),novelty:sum(choice.map(x=>x.share*x.item.noveltyDelta)),fit:sum(choice.map(x=>x.share*finite(x.item.segmentFit?.[seg.id])))};}
function storeOffer(state,store){const b=(state.businesses||[]).find(x=>x.id===store.businessID)||{};const concepts=globalThis.__capitalismTycoonModules?.storeEquipment,concept=concepts?.conceptOf(store)||{capacityMultiplier:1,qualityDelta:0,serviceDelta:0,noveltyDelta:0,repeatRateDelta:0,segmentFit:{},recipeMultipliers:{}};const p=(state.prefs||[]).find(x=>x.id===store.prefID)||{};const tenant=(state.tenants||[]).find(t=>t.id===store.tenantID)||{};const dept=id=>state.departments&&state.departments[id]?(.35+finite(state.departmentStaff?.[id],1)*.08):0;const cmo=finite(state.executives?.CMO?.skill,0),coo=finite(state.executives?.COO?.skill,0);const ads=dept('marketing')*.04+cmo/2200;const ops=dept('operations')*.03+coo/2400;const convenience=clamp(40+finite(p.traffic,1)*32+finite(store.operatingHours,3)*7,0,100);const quality=clamp(finite(b.quality)+finite(store.quality)*.35+ops*100+finite(concept.qualityDelta),0,100);const brandAwareness=clamp(finite(b.brand)+finite(store.brand)*.35+finite(state.companyReputation,0)*.18+ads*100,0,100);const brandTrust=clamp(brandAwareness*.65+quality*.25+finite(state.companyReputation,0)*.10,0,100);const wf=globalThis.__capitalismTycoonModules?.workforce;let baseCap=effectiveCapacity(store,b,p);let wfAdj=wf&&wf.TARGET_BUSINESS_IDS.indexOf(store.businessID)>=0?wf.storeAdjustment(state,store,baseCap):null;const serviceQuality=clamp(45+finite(state.employeeAbility,50)*.30+finite(state.employeeSatisfaction,55)*.18+ops*100+(wfAdj?wfAdj.serviceQualityAdjustment*100:0)+finite(concept.serviceDelta),0,100);const novelty=clamp(finite(b.dx,0)*.65+finite(b.quality,0)*.2+ads*25+finite(concept.noveltyDelta),0,100);const satisfaction=clamp(35+quality*.32+serviceQuality*.25+brandTrust*.16,0,100);const repeatRate=clamp(.12+satisfaction/180+brandTrust/380+finite(concept.repeatRateDelta),0,.88);const variableCostPerUnit=finite(b.unitCost)*(1+quality/1000)*(1-Math.min(.22,finite(b.efficiency)/260))/(1+dept('operations')*.04);const override=store.priceOverride;const price=typeof override==='number'&&Number.isFinite(override)&&override>0?override:Math.max(1,finite(b.price,1));const menuPlan=storeMenuPlan(state,store,b),activeMenuCount=menuPlan?.items?.length||1,menuComplexityMultiplier=activeMenuCount<=1?1:Math.max(.9,1-(activeMenuCount-1)*.025);return {id:store.id,kind:'player',store,business:b,pref:p,price,quality,brandAwareness,brandTrust,convenience,serviceQuality,novelty,capacity:Math.round((wfAdj?wfAdj.staffLimitedCapacity:baseCap)*(Object.prototype.hasOwnProperty.call(store,'capacity')?1:globalThis.__capitalismTycoonModules.tenantSiteSuitability.forStore(state,store).multiplier))*menuComplexityMultiplier*finite(concept.capacityMultiplier,1)*(finite(state.week)<finite(store.conceptTransitionUntilWeek)?0.82:1),workforceAdjustment:wfAdj,variableCostPerUnit,customerSatisfaction:satisfaction,repeatRate,menuPlan,activeMenuCount,menuComplexityMultiplier,concept};}
