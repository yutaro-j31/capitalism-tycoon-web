...
function fundFormation(state,gpCommit=null,promiseDecisions=null){
  ...
  const lps=arr(plan.lps).map(row=>({
    ...
    promiseID:row.promiseID?String(row.promiseID):null,promiseLabel:row.promiseLabel?String(row.promiseLabel):null,riskLabel:row.riskLabel?String(row.riskLabel):null,promiseAccepted:Boolean(row.promiseAccepted)
  }));
