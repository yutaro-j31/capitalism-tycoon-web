...
function exitReasonLabel(reason){
  ...
  if(reason==='buyer-score')return 'この買い手が求める改善水準に未達';
  if(reason==='buyer-hold')return 'この買い手が求める保有実績に未達';
  if(reason==='buyer-not-supported')return 'この売却先は利用できません';
  return reason?'実行条件未達':null;
}
function normalizeSaleBuyer(state,fundID,dealID,buyerID){
  const capability=portfolio?.exitBuyerCapabilities?.(state,fundID,dealID)?.find(row=>row.id===buyerID)||null;
  const preview=capability?.eligible?portfolio?.previewPortfolioExit?.(state,fundID,dealID,{method:'sale',buyerType:buyerID}):null;
  if(!preview?.ok)return {id:String(buyerID),label:String(capability?.label||buyerID),eligible:false,reason:capability?.reason||preview?.reason||'unavailable',reasonLabel:exitReasonLabel(capability?.reason||preview?.reason||'unavailable'),minScore:finite(capability?.minScore),minHoldWeeks:finite(capability?.minHoldWeeks)};
  const s=preview.settlement||{},gpPrincipalAndGain=finite(s.gpPrincipalAndGain),gpCarry=finite(s.gpCarry);
  return {id:String(buyerID),label:String(capability?.label||buyerID),eligible:true,reason:null,reasonLabel:null,quoteFactor:finite(preview.buyerFactor,1),exitEnterpriseValue:finite(preview.exitEnterpriseValue),grossProceeds:finite(preview.grossProceeds),moic:finite(preview.currentMOIC),irr:annualizedIRR(preview.currentMOIC,preview.holdingWeeks),personalCashProceeds:gpPrincipalAndGain+gpCarry,minScore:finite(capability?.minScore),minHoldWeeks:finite(capability?.minHoldWeeks)};
}
...
function performPortfolio(action,payload={}){
  ...
  if(action==='exit')return saveSuccessful(engine,portfolio.exitPortfolioCompany(state,fundID,dealID,{method:String(payload.method||'sale'),buyerType:String(payload.buyerType||'market')}));
...
modules.peUIAdapter=Object.freeze({NAVIGATION,getPEUIData,perform,performPortfolio,preferDrop,normalizePortfolio,normalizeExitPreview,normalizeExitScenario,normalizeExitRoute,normalizeSaleBuyer,thresholds:Object.freeze({DECISION_LIMIT,FINAL_BID_URGENT_WEEKS,INVESTMENT_RISK_FRACTION,DEADLINE_CRITICAL_WEEKS,DD_OPPORTUNITY_WEEKS,NETWORK_WARNING_MARGIN}),__installed:true});
