...
function renderRow(engine,p){
 ...
 const loan=loanFor(engine,p.id),owner=p.owner==='company'?'会社':'個人',enabled=Boolean(p.realEstate?.rentalEnabled);
 ...
 return `<article class="item real-estate-property" data-real-estate-property="${esc(p.id)}"><div><div class="button-row"><h3>${esc(p.name||p.id)}</h3><span class="badge">${owner}</span>${enabled?'<span class="badge good">賃貸中</span>':'<span class="badge">未稼働</span>'}</div><p>${esc(p.buildingType||p.kind||'土地')} · 評価 ${money(m.valuation)}</p><div class="item-metrics"><span>土地 ${money(m.landValue)}</span><span>建物 ${money(m.buildingValue)}</span><span>稼働率 ${pct(m.occupancy)}</span><span>表面利回り ${pct(m.grossYield)}</span><span>実質利回り ${pct(m.netYield)}</span><span>担保余力 ${money(m.collateralAvailable)}</span></div>${loan?`<p class="muted">借入残高 ${money(loan.outstandingPrincipal)} · 金利 ${pct(loan.interestRate)} · 残り${loan.remainingWeeks}週</p>`:''}</div><div class="button-row">${rentAction}${borrow}${repay}${sell}</div></article>`;
}
function render(engine){const rows=owned(engine);return `<section class="card real-estate-manager" data-real-estate-manager><div class="card-head"><div><h2>不動産経営</h2><p>土地・建物評価、賃貸収支、担保融資を一括管理します。</p></div><span class="badge good">${rows.length}件</span></div><div class="card-body">${rows.length?rows.map(p=>renderRow(engine,p)).join(''):'<div class="empty">保有物件はありません。資産タブの物件購入から取得できます。</div>'}</div></section>`;}
...
install();
modules.realEstateUI=Object.freeze({install,inject,render,renderRow,owned,loanFor});
})();