...
function developmentFor(engine,propertyID){const s=engine?.g?.realEstateDevelopment;return {project:(s?.projects||[]).find(x=>x.propertyID===propertyID&&x.status==='active')||null,policy:(s?.policies||[]).find(x=>x.propertyID===propertyID&&x.status==='active')||null};}
function renderRow(engine,p){
 const m=engine.getPropertyInvestmentMetrics?.(p.id);if(!m)return '';
 const loan=loanFor(engine,p.id),event=latestEvent(engine,p.id),profile=m.marketProfile||engine.getPropertyMarketProfile?.(p.id),owner=p.owner==='company'?'会社':'個人',enabled=Boolean(p.realEstate?.rentalEnabled),dev=developmentFor(engine,p.id);
 ...
 const sell=!loan&&!dev.project?`<button class="btn danger small" data-real-estate-action="sell" data-property-id="${esc(p.id)}">売却</button>`:'<span class="badge warn">借入・工事中は売却不可</span>';
 ...
 const projectLine=dev.project?`<p class="muted">${dev.project.kind==='development'?'開発':'改修'}進行 ${(finite(dev.project.progress)*100).toFixed(0)}% · 総額 ${money(dev.project.totalCost)}</p>`:'';
 const developmentActions=modules.realEstateDevelopment?(dev.project?'':`<button class="btn secondary small" data-real-estate-action="renovate" data-property-id="${esc(p.id)}">改修</button><button class="btn secondary small" data-real-estate-action="develop" data-property-id="${esc(p.id)}">再開発</button>`)+(dev.policy?`<button class="btn secondary small" data-real-estate-action="cancel-insurance" data-policy-id="${esc(dev.policy.policyID)}">保険解約</button>`:`<button class="btn secondary small" data-real-estate-action="insure" data-property-id="${esc(p.id)}">保険加入</button>`):developmentFailed?'<span class="badge warn">拡張読込失敗</span>':'<span class="badge">拡張読込中</span>';
 return `<article class="item real-estate-property" data-real-estate-property="${esc(p.id)}"><div><div class="button-row"><h3>${esc(p.name||p.id)}</h3><span class="badge">${owner}</span>${marketBadges}${enabled?'<span class="badge good">賃貸中</span>':'<span class="badge">未稼働</span>'}</div><p>${esc(p.buildingType||p.kind||'土地')} · 評価 ${money(m.valuation)}</p><div class="item-metrics"><span>土地 ${money(m.landValue)}</span><span>建物 ${money(m.buildingValue)}</span><span>稼働率 ${pct(m.occupancy)}</span><span>需要倍率 ${profile?pct(profile.demandMultiplier):'—'}</span><span>賃料倍率 ${profile?pct(profile.rentMultiplier):'—'}</span><span>表面利回り ${pct(m.grossYield)}</span><span>実質利回り ${pct(m.netYield)}</span><span>担保余力 ${money(m.collateralAvailable)}</span></div>${eventLine}${projectLine}${loan?`<p class="muted">借入残高 ${money(loan.outstandingPrincipal)} · 金利 ${pct(loan.interestRate)} · 残り${loan.remainingWeeks}週</p>`:''}</div><div class="button-row">${rentAction}${borrow}${repay}${developmentActions}${sell}</div></article>`;
}
function render(engine){const rows=owned(engine),events=engine?.g?.realEstate?.marketEvents||[],history=engine?.g?.realEstateDevelopment?.history||[];return `<section class="card real-estate-manager" data-real-estate-manager><div class="card-head"><div><h2>不動産経営</h2><p>地域需要、賃貸、担保融資、開発、改修、保険、固定資産税を管理します。</p></div><span class="badge good">${rows.length}件</span></div><div class="card-body">${events.length?`<p class="muted">市場イベント ${events.length}件 · 開発会計履歴 ${history.length}件</p>`:''}${rows.length?rows.map(p=>renderRow(engine,p)).join(''):'<div class="empty">保有物件はありません。資産タブの物件購入から取得できます。</div>'}</div></section>`;}
let queued=false,installed=false;
function inject(){queued=false;loadDevelopment();const engine=bridge.getEngine(),screen=document.querySelector('[data-screen="assets"]');if(!engine||!screen)return;const html=render(engine),current=screen.querySelector('[data-real-estate-manager]');if(current)current.outerHTML=html;else screen.insertAdjacentHTML('afterbegin',html);}
...
function act(button){const engine=bridge.getEngine();if(!engine)return;const id=button.dataset.propertyId,action=button.dataset.realEstateAction;
 ...
 else if(action==='renovate')engine.startPropertyDevelopment?.(id,'renovation');
 else if(action==='develop')engine.startPropertyDevelopment?.(id,'development');
 else if(action==='insure')engine.buyPropertyInsurance?.(id);
 else if(action==='cancel-insurance')engine.cancelPropertyInsurance?.(button.dataset.policyId);
...
install();
modules.realEstateUI=Object.freeze({install,inject,render,renderRow,owned,loanFor,latestEvent,developmentFor,loadDevelopment});
})();
