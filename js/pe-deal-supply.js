...
// 週次の案件供給。広域市場の基礎供給は従来どおり年4件固定。
// ただしTrust 80+の関係がある場合は、その関係が持ち込む紹介案件を四半期ごとに1件まで
// 別枠で追加する。これによりネットワーク100でも板が常時2件前後に張り付く問題を解消しつつ、
// 基礎市場の較正・DD枠・MAX_PE_TARGETS(8)・独占案件40% ceilingは維持する。
function processSupplyWeek(state,week){
  ...
  // 紹介資格はこの供給サイクル開始時点のTrustで固定する。後段で通常案件が独占化して
  // Trustを消費しても、「今期すでに持ち込まれた紹介」が消えることはない。
