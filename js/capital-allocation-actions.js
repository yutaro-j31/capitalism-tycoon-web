// Phase 8C-10/8C-14: execute recommended debt repayment safely, explain score impact, and track recovery thresholds.
...
function impact(instance,policyId){
  ...
  const amount=value.debtExecutable;
  const after=afterDebtRepayment(current,amount);
  const beforeBreakdown=policy.executionScoreBreakdown(value.policy,current);
  const afterBreakdown=policy.executionScoreBreakdown(value.policy,after);
  ...
  const recoveryTarget=recoveryTargetFor(value.policy,current);
  return{policy:value.policy,policyName:value.policyName,amount,beforeScore,afterScore,improvement,canExecute:amount>0&&improvement>=0,currentMetrics:current,afterMetrics:after,beforeBreakdown,afterBreakdown,componentEffects,largestImprovement:improvements[0]||null,largestRegression:regressions[0]||null,recoveryTarget};
}
function executeDebt(instance,policyId){
  ...
  const scoreImpact=impact(instance,policyId);
...
function render(instance=modules.playerEngineBridge?.getEngine?.()){
  ...
  const key=renderKey(`${instance.g.week}|${recommended}|${value.debtRequested}|${value.debtExecutable}|${value.capacity.cash}|${value.capacity.debt}|${scoreImpact.beforeScore}|${scoreImpact.afterScore}|${breakdownKey}|${recoveryKey}|${warnings.join('|')}`);
...
modules.capitalAllocationActions=Object.freeze({preview,attainmentFor,afterDebtRepayment,recoveryTargetFor,impact,executeDebt,renderKey,render,enhance,installUI,__installed:true});
installUI();
})(0);
