// Phase 8D-15/16: deterministic recovery-funding alternatives and board trade-off leaders.
...
function render(instance=modules.playerEngineBridge?.getEngine?.()){
 ...
 const key=renderKey(`${value.week}|${value.targetPolicy}|${value.rows.map(row=>`${row.id}:${row.targetReached}:${row.projectedScore}:${row.assetFunding}:${row.borrowing}:${row.propertyCount}:${row.disposalGainLoss}`).join('|')}`);
 return `<section class="card" data-capital-allocation-recovery-funding-options="1" data-capital-allocation-recovery-funding-options-render-key="${key}"><div class="card-head"><div><h2>回復資金・代替案比較</h2><p>同じ目標スコアに対し、借入と資産売却のトレードオフを比較します。自動売却・自動借入は行いません。</p></div><span class="badge ${value.reachableCount?'good':'bad'}">Phase 8D-16 · ${value.reachableCount}/4案到達</span></div><div class="card-body"><div class="kpi-grid mini">${optionRows}</div><h3>取締役会の比較軸</h3><div class="kpi-grid mini">${leaderRows}</div><p><strong>目標方針：</strong>${esc(value.targetPolicyName||'到達方針なし')} ／ <strong>目標：</strong>${value.targetScore}点</p></div></section>`;
}
...
modules.capitalAllocationRecoveryFundingOptions=Object.freeze({OPTION_ORDER,borrowingAmount,selectedProfile,statusFor,optionFromAllocation,recommendedOption,compareBy,leader,tradeoffLeaders,matrix,frontier,renderKey,render,enhance,installUI,__installed:true});
installUI();
})(0);
