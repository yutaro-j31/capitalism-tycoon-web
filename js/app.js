// Script boundary: index.html original inline script #1 (classic JavaScript)
(()=>{'use strict';
...
function installExpansion(TycoonEngine){
  ...
  TycoonEngine.prototype.draftPlayer=function(teamID,candidateID){const t=this.g.sportsTeams.find(x=>x.id===teamID),c=this.g.sportsDraftCandidates.find(x=>x.id===candidateID);if(!t||!c)return false;const cashKey=t.owner==='company'?'companyCash':'personalCash';if(this.g[cashKey]<c.expectedSalary)return this.fail('契約資金が不足しています。');this.g[cashKey]-=c.expectedSalary;t.teamStrength=clamp(t.teamStrength+c.potential/35,0,100);t.roster=(t.roster||[]);t.roster.push(c);this.g.sportsDraftCandidates=this.g.sportsDraftCandidates.filter(x=>x.id!==candidateID);this.notify(`${t.name}が${c.name}を指名しました。`,'success');this.save();this.emit();return true;};
  TycoonEngine.prototype.tradePlayer=function(teamID,assetID){const t=this.g.sportsTeams.find(x=>x.id===teamID),a=this.g.sportsTradeMarket.find(x=>x.id===assetID);if(!t||!a)return false;const cashKey=t.owner==='company'?'companyCash':'personalCash';if(this.g[cashKey]<a.askingPrice)return this.fail('獲得資金が不足しています。');this.g[cashKey]-=a.askingPrice;t.teamStrength=clamp(t.teamStrength+a.rating/28,0,100);t.roster=(t.roster||[]);t.roster.push(a);this.g.sportsTradeMarket=this.g.sportsTradeMarket.filter(x=>x.id!==assetID);this.notify(`${t.name}が${a.playerName}をトレード獲得しました。`,'success');this.save();this.emit();return true;};
  TycoonEngine.prototype.signForeignPlayer=function(teamID){const t=this.g.sportsTeams.find(x=>x.id===teamID);if(!t)return false;const cost=rand(50000000,220000000),cashKey=t.owner==='company'?'companyCash':'personalCash';if(this.g[cashKey]<cost)return this.fail('補強資金が不足しています。');this.g[cashKey]-=cost;t.teamStrength=clamp(t.teamStrength+rand(3,8),0,100);this.notify(`${t.name}が外国人選手を補強しました。`,'success');this.save();this.emit();return true;};
...
const ui = {
  ...
  selectedAccount: 'personal', stockSector: 'all', assetTab: 'property', officeTab: 'overview',
  reportMetric: 'profit', strategyTab: 'supply', personalTab: 'development', legacyTab: 'succession', showSetup: !engine.g.configured
};
...
function renderMarket() {
  const g=engine.g,sectors=['all',...new Set(g.market.map(s=>s.sector))],stocks=g.market.filter(s=>ui.stockSector==='all'||s.sector===ui.stockSector);
  ...
  <div class="market-table"><div class="market-row header"><span>銘柄</span><span>株価</span><span>前週比</span><span>時価総額</span><span>PER/PBR</span><span>保有</span><span></span></div>${stocks.map(s=>{const change=(s.price-s.previous)/s.previous,h=(ui.selectedAccount==='company'?g.companyStocks:g.personalStocks)[s.id];return `<div class="market-row"><span><strong>${esc(s.name)}</strong><small>${esc(s.id)} · ${esc(s.sector)}</small></span><span>${yen(s.price)}</span><span class="${change>=0?'up':'down'}">${change>=0?'+':''}${pct(change)}</span><span>${compactYen(s.marketCap)}</span><span>${s.per?`${s.per.toFixed(1)}倍`:'—'} / ${s.pbr?`${s.pbr.toFixed(1)}倍`:'—'}</span><span>${h?`${num(h.qty)}株<br><small>平均${yen(h.avg)}</small>`:'—'}</span><span class="button-row">${btn('買う','buy-stock',{kind:'primary small',data:`data-id="${s.id}"`})}${btn('売る','sell-stock',{kind:'ghost small',data:`data-id="${s.id}"`,disabled:!h})}${btn(g.favoriteStockIds.includes(s.id)?'★':'☆','favorite-stock',{kind:'icon-button',data:`data-id="${s.id}"`})}</span></div>`}).join('')}</div>${renderCapitalMarketExpansion()}${renderEarningsSection()}`;
}
...
function renderCashflowForecast(){const g=engine.g,r=g.lastReport,weekly=r?.profit||0;return `<div class="forecast">${Array.from({length:8},(_,i)=>{const v=g.companyCash+weekly*(i+1);return `<div><span>${i+1}週後</span><strong class="${v<0?'down':''}">${compactYen(v)}</strong></div>`}).join('')}</div><p class="muted">直近週の利益が継続すると仮定した簡易予測です。</p>`;}

function renderReport(){const g=engine.g,r=g.lastReport;return `${card('決算・経営分析',`<div class="filters"><label>チャート<select data-bind="reportMetric"><option value="profit" ${ui.reportMetric==='profit'?'selected':''}>週次利益</option><option value="sales" ${ui.reportMetric==='sales'?'selected':''}>週次売上</option><option value="value" ${ui.reportMetric==='value'?'selected':''}>企業価値</option><option value="networth" ${ui.reportMetric==='networth'?'selected':''}>個人純資産</option></select></label></div><canvas id="report-chart" height="180"></canvas>`)}${r?`<div class="grid three">${card('損益計算',`${stat('売上高',yen(r.sales))}${stat('賃料収入',yen(r.rentIncome))}${stat('株式配当',yen(r.stockIncome))}${stat('費用',yen(r.expenses))}${stat('最終利益',yen(r.profit))}`)}${card('費用内訳',`${stat('利息',yen(r.interest))}${stat('オフィス',yen(r.officeCost))}${stat('配当支払',yen(r.dividend))}${stat('減価償却',yen(r.propertyDepreciation))}${stat('法人税',yen(r.tax))}`)}${card('企業指標',`${stat('企業価値',compactYen(engine.companyValue()))}${stat('自己資本相当',compactYen(engine.companyValue()-g.companyDebt))}${stat('創業者持株',pct(g.founderOwnershipRatio))}${stat('株価',g.publicCompany?yen(g.stockPrice):'未上場')}${stat('信用力',g.companyCredit.toFixed(0))}`)}</div>`:empty('1週進めると決算が作成されます。')}${card('週次履歴',`<div class="history-table">${g.reports.slice(-30).reverse().map(x=>`<div><span>第${x.week}週</span><span>${compactYen(x.sales)}</span><strong class="${x.profit>=0?'up':'down'}">${compactYen(x.profit)}</strong><span>${compactYen(g.companyValueHistory[g.companyValueHistory.length-1-(g.week-x.week)]||0)}</span></div>`).join('')}</div>`)}`;}
