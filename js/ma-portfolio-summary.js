...
function activeSub(s){return (s?.status||'active')==='active';}
function activeSubs(g){return arr(g?.maSubsidiaries).filter(activeSub).slice().sort((a,b)=>String(a?.id||a?.name||'').localeCompare(String(b?.id||b?.name||'')));}
function goodwillByID(g,id,fallback){const records=arr(g?.goodwillRecords);const any=id?records.find(r=>r?.id===id):null;if(any?.status==='disposed')return 0;return Math.max(0,nf(any?.carryingValue,any?nf(any?.goodwillBookValue,nf(any?.amount,0)):fallback));}
