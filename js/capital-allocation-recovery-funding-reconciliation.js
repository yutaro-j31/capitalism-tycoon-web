...
const EngineClass=modules.engine.TycoonEngine,readiness=modules.capitalAllocationRecoveryFundingReadiness,policy=modules.capitalAllocationPolicy,OPTION_IDS=Object.freeze([...modules.capitalAllocationRecoveryFundingOptions.OPTION_ORDER]);
...
function candidates(instance,targetScore=70){return OPTION_IDS.map(optionId=>readiness.snapshot(instance,targetScore,optionId));}
function pin(instance,targetScore=70,optionId='recommended'){const value=readiness.snapshot(instance,targetScore,optionId);if(!value.ready)return null;pins.set(instance,value);return value;}
...
function render(instance=modules.playerEngineBridge?.getEngine?.()){
 ...
 if(!fixed){const plans=candidates(instance,70),rows=plans.map(plan=>`<div class="stat"><span>${esc(plan.optionName||plan.optionId||'資金案')}</span><strong>${plan.targetReached?`${plan.projectedScore}点`:'到達不能'}</strong><small>${esc(plan.ready?'固定可能':plan.reason)}</small><button class="btn small" data-capital-allocation-funding-pin="70" data-capital-allocation-funding-option="${esc(plan.optionId||'recommended')}"${plan.ready?'':' disabled'}>この案を固定</button></div>`).join(''),key=`empty:${plans.map(plan=>`${plan.optionId}:${plan.fingerprint}:${plan.ready}`).join('|')}`;return `<section class="card" data-capital-allocation-recovery-funding-reconciliation="1" data-capital-allocation-recovery-funding-reconciliation-key="${esc(key)}"><div class="card-head"><div><h2>回復資金・実行後照合</h2><p>4つの資金案から計画を一時固定し、手動取引後の進捗と会計記録を照合します。固定内容はセーブされません。</p></div><span class="badge warn">Phase 8D-20 · 未固定</span></div><div class="card-body"><div class="kpi-grid mini">${rows}</div></div></section>`;}
...
modules.capitalAllocationRecoveryFundingReconciliation=Object.freeze({OPTION_IDS,moneyClose,statusLabel,toneFor,currentPolicy,sourceFor,transactionId,transactionsSince,matchesStep,evidenceForStep,reconcileStep,reconcile,candidates,pin,clear,pinned,render,enhance,installUI,__installed:true});
installUI();
})(0);