...
function campaignRows(state){return(Array.isArray(state?.mediaAdvertisingCampaigns)?state.mediaAdvertisingCampaigns:[]).map(normalizeCampaign).filter(Boolean).slice(-80);}
function ensure(state){state.mediaAdvertisingCampaigns=campaignRows(state);return state.mediaAdvertisingCampaigns;}
function isActive(row,week){return finite(week)>row.startedWeek&&finite(week)<=row.endsWeek;}
function activeCampaigns(state,businessID){return campaignRows(state).filter(row=>(!businessID||row.businessID===businessID)&&isActive(row,state.week));}
function remainingWeeks(row,week){week=finite(week);return Math.max(0,row.endsWeek-Math.max(week,row.startedWeek)+(week>row.startedWeek?1:0));}
...
Engine.prototype.startMediaAdvertisingCampaign=function(businessID,mediaID,budget){const work=()=>{const business=this.business?.(businessID),media=MEDIA[mediaID];budget=Number(budget);if(!business||!media||!Number.isFinite(budget)||budget<media.cost||budget%10000!==0)return this.fail?.('事業・媒体・予算が不正です。')??false;const campaigns=campaignRows(this.g);if(campaigns.some(row=>row.businessID===businessID&&remainingWeeks(row,this.g.week)>0))return this.fail?.('この事業では広告キャンペーンを実施中です。')??false;if(finite(this.g.companyCash)<budget)return this.fail?.('会社資金が不足しています。')??false;this.g.mediaAdvertisingCampaigns=campaigns;const week=Math.max(1,Math.floor(finite(this.g.week,1))),id=`media-ad-${businessID}-${mediaID}-${week}`;this.g.companyCash-=budget;finance.event(this.g,'advertising',budget,{cashEffect:-budget,profitEffect:-budget,assetEffect:0,businessID,sourceType:'mediaAdvertisingCampaign',sourceID:id,operationID:id,idempotencyKey:id,description:`${business.name} ${media.name}`});this.g.mediaAdvertisingCampaigns.push({id,businessID,mediaID,budget,startedWeek:week,endsWeek:week+media.durationWeeks});this.notify?.(`${media.name}を開始しました。効果期間は${media.durationWeeks}週間です。`,'success');this.save?.();this.emit?.();return true;};return typeof this.runTransaction==='function'?this.runTransaction(work,'change',{source:'mediaAdvertisingCampaign'}):work();};
