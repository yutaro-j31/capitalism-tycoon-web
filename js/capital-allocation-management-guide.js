...
const NAV_ITEMS=Object.freeze([
 Object.freeze({id:'marketOverview',label:'市場概要',selector:'[data-market-overview],[data-market-summary],[data-market-ui]'}),
 Object.freeze({id:'shareholderReturns',label:'株主還元',selector:'[data-shareholder-returns-ui]'}),
 Object.freeze({id:'capitalPolicy',label:'資本配分スコア・方針',selector:'[data-capital-allocation-score-ui],[data-capital-allocation-policy-ui]'}),
 Object.freeze({id:'boardMemo',label:'取締役会メモ・ストレステスト',selector:'[data-capital-allocation-decision-memo],[data-capital-allocation-stress-test],[data-capital-allocation-resilience-memo]'}),
 Object.freeze({id:'recoveryPlan',label:'回復資金計画',selector:'[data-capital-allocation-recovery-funding],[data-capital-allocation-recovery-funding-options]'}),
 Object.freeze({id:'readiness',label:'実行前確認',selector:'[data-capital-allocation-recovery-funding-readiness]'}),
 Object.freeze({id:'reconciliation',label:'実行後照合',selector:'[data-capital-allocation-recovery-funding-reconciliation]'}),
 Object.freeze({id:'outcome',label:'結果検証',selector:'[data-capital-allocation-recovery-funding-outcome]'}),
 Object.freeze({id:'managementGuide',label:'経営判断ガイド',selector:'[data-capital-allocation-management-guide]'})
]);
...
function firstExisting(root,selector){try{return root?.querySelector?.(selector)||null;}catch(_){return null;}}
function navSections(root=document){
 const rec=recommendation(modules.playerEngineBridge?.getEngine?.()),focusTarget=firstExisting(root,rec.focus);
 return NAV_ITEMS.map(item=>({item,target:firstExisting(root,item.selector)})).filter(row=>row.target).map(row=>Object.freeze({id:row.item.id,label:row.item.label,selector:row.item.selector,recommended:Boolean(focusTarget&&(row.target===focusTarget||row.target.contains?.(focusTarget)||focusTarget.contains?.(row.target)))}));
}
function renderNav(root=document){
 const rows=navSections(root);if(!rows.length)return'';
 const buttons=rows.map(row=>`<button class="btn small${row.recommended?' primary':''}" type="button" data-capital-allocation-section-nav-target="${esc(row.selector)}" aria-current="${row.recommended?'true':'false'}">${esc(row.label)}${row.recommended?' · 推奨':''}</button>`).join('');
 return `<nav class="capital-allocation-section-nav" data-capital-allocation-section-nav="1" aria-label="市場画面セクションナビ" role="navigation"><span>セクション</span><div class="capital-allocation-section-nav-buttons" role="list">${buttons}</div></nav>`;
}

function enhanceNav(){if(typeof document==='undefined')return false;const screen=document.getElementById('screen');if(!screen)return false;const old=screen.querySelector?.('[data-capital-allocation-section-nav]'),html=renderNav(screen);if(!html){old?.remove?.();return false;}if(old&&old.outerHTML===html)return false;if(old){old.outerHTML=html;return true;}const host=screen.querySelector?.('[data-shareholder-returns-ui]')||screen.querySelector?.('[data-capital-allocation-score-ui]')||screen.querySelector?.('.market-table')||screen.firstElementChild;if(host?.insertAdjacentHTML){host.insertAdjacentHTML('beforebegin',html);return true;}screen.insertAdjacentHTML?.('afterbegin',html);return true;}
function enhance(){if(typeof document==='undefined')return false;const screen=document.getElementById('screen');if(!screen)return false;const old=screen.querySelector?.('[data-capital-allocation-management-guide]'),html=render();if(!html){old?.remove?.();enhanceNav();return false;}const desired=(html.match(/data-capital-allocation-management-guide-key="([^"]+)"/)||[])[1]||'',current=String(old?.getAttribute?.('data-capital-allocation-management-guide-key')||'');let changed=false;if(old&&current===desired)changed=false;else if(old){old.outerHTML=html;changed=true;}else{const host=screen.querySelector?.('[data-capital-allocation-policy-ui]')||screen.querySelector?.('[data-shareholder-returns-ui]');if(host){host.insertAdjacentHTML?.('afterend',html);changed=true;}else{screen.insertAdjacentHTML?.('beforeend',html);changed=true;}}return enhanceNav()||changed;}
function schedule(){enhance();}
function focusTarget(selector){const target=selector&&document.querySelector?.(selector);if(!target)return false;target.setAttribute?.('tabindex',target.getAttribute?.('tabindex')||'-1');target.scrollIntoView?.({block:'start',behavior:reducedMotion()?'auto':'smooth'});target.focus?.({preventScroll:true});return true;}
...
function installUI(){if(typeof document==='undefined')return;document.addEventListener?.('click',event=>{const button=event.target?.closest?.('[data-capital-allocation-guide-focus],[data-capital-allocation-section-nav-target]');if(!button)return;const selector=button.dataset.capitalAllocationGuideFocus||button.dataset.capitalAllocationSectionNavTarget;if(focusTarget(selector))event.preventDefault?.();});const root=document.getElementById('app');registerEnhancer({id:'capital-allocation-management-guide',enhance:enhance});schedule();}
modules.capitalAllocationManagementGuide=Object.freeze({NAV_ITEMS,selectedTarget,cooldownAction,fixedPlanRecommendation,recommendation,navSections,renderNav,focusTarget,render,enhance,installUI,__installed:true});installUI();
})(0);
