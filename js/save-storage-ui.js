...
function renderCard(){
 ...
 node.innerHTML=`<div class="card-head"><div><h2>セーブ容量</h2><p>iPhoneの保存上限に合わせて古い履歴を整理します。会社・個人資産と会計累計は保持されます。</p></div><span class="badge ${ok?'good':'danger'}">${esc(label)}</span></div><div class="card-body"><div class="kpi-grid mini"><div class="stat"><span>保存状態</span><strong>${ok?'保存可能':'要バックアップ'}</strong></div><div class="stat"><span>保存サイズ</span><strong>${esc(size)}</strong><small>${esc(original)}</small></div><div class="stat"><span>整理した会計明細</span><strong>${removed.toLocaleString('ja-JP')}件</strong></div><div class="stat"><span>セーブ形式</span><strong>v${storage.SAVE_VERSION}</strong><small>${esc(storage.SAVE_KEY)}</small></div></div>${ok?'':'<p class="empty">以前のセーブは残っています。JSON書き出しを保存してから再試行してください。</p>'}</div>`;
 return true;
}
...
modules.saveStorageUI=Object.freeze({MODE_LABELS,bytes,engine,info,toast,renderCard,handleSaveClick,install,__installed:true});
install();
})();