// Phase 6A-5C / Phase 8A-2: capture the app engine and install product innovation roadmaps.
(function(){'use strict';
...
function renderSection(instance=activeEngine){
  ...
  const key=renderKey(rows);return `<section class="card product-innovation-panel" data-product-innovation-ui="1" data-product-innovation-render-key="${key}" aria-live="polite"><div class="card-head"><div><h2>プロダクト・イノベーション</h2><p>短期投資ではなく、複数週の開発計画と特許実装で競争優位を構築します。</p></div><span class="badge good">Phase 8A-2</span></div><div class="card-body"><div class="grid two">${rows}</div></div></section>`;
}
