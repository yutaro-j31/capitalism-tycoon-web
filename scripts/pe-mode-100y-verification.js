...
function runOnce({ seed, skillID, weeks, sampleEvery = 520 }) {
  ...
  for (let w = 0; w < weeks; w++) {
    ...
    // T26-3: 天井の判定に使うのは「プレイヤーに帰属する純資産」。運用資産(AUM)は
    // LPと共同投資家の資本を含むので、同じ数字として扱ってはいけない（両方を残して並べる）。
    if ((w + 1) % sampleEvery === 0) samples.push({
      ...
      teamPayrollPaid: e.g.peFirm.funds.reduce((sum, f) => sum + finite(f.teamPayrollPaid), 0),
  ...
  return {
    ...
    coinvestContributed: Math.max(0, finite(g.peFirm?.coinvestContributed)),
    coinvestCapital: Math.max(0, finite(g.peFirm?.coinvestCapital)),
