...
for(const run of runs){const key=groupKey(run);if(!groups.has(key))groups.set(key,[]);groups.get(key).push(run);}
const reasons=new Map();
for(const run of runs){const reason=run.failureReason||'survived';reasons.set(reason,(reasons.get(reason)||0)+1);}

const lines=['# Playwright大量自動プレイ分析','',`- 実行数: ${runs.length}`,'- ビューポート: 390×844（iPhone相当）','- 判定: 実ゲーム画面を操作し、localStorageセーブを集計','', '## 戦略・難易度別サマリー','', '| ブラウザ / 難易度 / 戦略 | 実行 | 生存率 | 到達週中央値 | 最低現金中央値 | 累計利益中央値 | JSエラー実行 |','|---|---:|---:|---:|---:|---:|---:|'];
for(const [key,items] of [...groups.entries()].sort()){
  const survived=items.filter(r=>!r.gameOver&&!r.failureReason).length;
  const errorRuns=items.filter(r=>(r.consoleErrors?.length||0)+(r.pageErrors?.length||0)>0||r.runnerError).length;
...
for(const run of runs){if(!strategyGroups.has(run.strategy))strategyGroups.set(run.strategy,[]);strategyGroups.get(run.strategy).push(run);}
...
const earlyFailures=runs.filter(r=>r.gameOver&&Number(r.weeksCompleted)<=26).length;
if(earlyFailures/runs.length>=0.2)warnings.push(`26週以内の早期終了が${pct(earlyFailures/runs.length)}あり、初期導線・初期固定費・資金警告の改善が必要`);
